...
// 2) implements your own eFile.c system with no code pasted in from other
// sources 3) streams real-time data from robot onto disk 4) supports multiple
// file reads/writes 5) has an interpreter that demonstrates features 6)
// interactive with UART input, and switch input
...
int realmain(void) {  // lab 4 real main
  ...
  ADC0_InitTimer0ATriggerSeq0(
      0, 50, &Producer);  // start ADC sampling, channel 0, PE3, 50 Hz
  ADC_Init(3);            // sequencer 3, channel 3, PE0, sampling in DAS()
  OS_AddPeriodicThread(&DAS, 10 * TIME_1MS,
                       1);  // 100Hz real time sampling of PE0

  // attach background tasks
  OS_AddPeriodicThread(&disk_timerproc, TIME_1MS,
