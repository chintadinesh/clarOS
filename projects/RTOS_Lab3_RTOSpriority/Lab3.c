...
void Consumer(void) {
  ...
  ADC0_InitTimer0ATriggerSeq0(
      1, FS, &Producer);  // start ADC sampling, channel 1, PE2, 400 Hz
...
int Testmain5(void) {  // Testmain5
  ...
  NumCreated +=
      OS_AddThread(&Thread2e, 128, 0);  // Lab 3 set to highest priority
...
void Thread7(void) {             // foreground thread
  ...
  // Jitter(MaxJitter2, JitterSize2, JitterHistogram2);  // print jitter of
  // second thread
  UART_OutString("\n\r\n\r");
  OS_Kill();
}
...
int Testmain7(void) {  // Testmain7  Lab 3
  ...
  OS_AddPeriodicThread(&Signal1, (799 * TIME_1MS) / 1000,
                       0);  // 0.799 ms, higher priority
  OS_AddPeriodicThread(&Signal2, (1111 * TIME_1MS) / 1000,
                       1);  // 1.111 ms, lower priority
  NumCreated = 0;
  ...
  NumCreated +=
      OS_AddThread(&Thread6, 128, 5);  // idle thread to keep from crashing
