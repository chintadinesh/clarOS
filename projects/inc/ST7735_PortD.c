...
// Outputs: None
// Notes: ...
void Delay1ms_PortD(uint32_t n) {
...
void static commonInit(const uint8_t *cmdList) {
  ...
  GPIO_PORTD_AMSEL_R &=
      ~0xC2;  // disable analog functionality on PD1,6,7 (PA3,6,7)
  TFT_CS = TFT_CS_LOW;
  RESET = RESET_HIGH;
  Delay1ms_PortD(500);
  RESET = RESET_LOW;
  ...
  GPIO_PORTD_AMSEL_R &=
      ~0x0B;  // disable analog functionality on PD0,1,3 (PA2,3,5)
  ...
  SSI3_CPSR_R =
      (SSI3_CPSR_R & ~SSI_CPSR_CPSDVSR_M) + 10;  // must be even number
    ...
    for (j = 0; j < 8; j++) {
      if (line & 0x1) {
        if (size == 1)  // default size
          ...
          ST7735_FillRect_PortD(x + (i * size), y + (j * size), size, size,
                                textColor);
        }
...
void ST7735_OutUDec_PortD(uint32_t n) {
  ...
  ST7735_DrawString_PortD(StX_PortD, StY_PortD, Message_PortD,
                          StTextColor_PortD);
  StX_PortD = StX_PortD + Messageindex_PortD;
  if (StX_PortD > 20) {
    StX_PortD = 20;
    ST7735_DrawCharS_PortD(StX_PortD * 6, StY_PortD * 10, '*', ST7735_RED,
                           ST7735_BLACK, 1);
  }
}
...
void ST7735_PlotClear_PortD(int32_t ymin, int32_t ymax) {
  ST7735_FillRect_PortD(0, 32, 128, 128,
                        ST7735_Color565_PortD(228, 228, 228));  // light grey
  if (ymax > ymin) {
    Ymax_PortD = ymax;
    Ymin_PortD = ymin;
...
void ST7735_PlotNextErase_PortD(void) {
  ...
  ST7735_DrawFastVLine_PortD(X_PortD, 32, 128,
                             ST7735_Color565_PortD(228, 228, 228));
}
...
void ST7735_OutChar_PortD(char ch) {
  if ((ch == 10) || (ch == 13) || (ch == 27)) {
    ...
    ST7735_DrawString_PortD(0, StY_PortD, "                     ",
                            StTextColor_PortD);
    return;
  }
  ST7735_DrawCharS_PortD(StX_PortD * 6, StY_PortD * 10, ch, StTextColor_PortD,
                         ST7735_BLACK, 1);
  StX_PortD++;
  if (StX_PortD > 20) {
    StX_PortD = 20;
...
// Output: none
// ********************************************************
void ST7735_SetTextColor_PortD(uint16_t color) { StTextColor_PortD = color; }
...
int fgetc_PortD(FILE *f) { return 0; }
...
void plotLineX_PortD(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                     uint16_t color) {
  int32_t d = 0;
  int32_t dx = x2 - x1;
  int32_t dy = y2 - y1;
...
void plotLineY_PortD(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                     uint16_t color) {
  int32_t d = 0;
  int32_t dx = x2 - x1;
  int32_t dy = y2 - y1;
