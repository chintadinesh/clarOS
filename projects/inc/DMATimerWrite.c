...
void DMA_Init(uint16_t period, uint32_t *destination) {
  ...
  UDMA_CHMAP1_R = (UDMA_CHMAP1_R & 0xFFFFFFF0) |
                  0x00000003;  // timer5A, channel 8, encoding 3
  ...
  UDMA_REQMASKCLR_R =
      BIT8;  // allow the µDMA controller to recognize requests for this channel
  Timer5A_Init(period);
  Status = IDLE;
}
...
void static setRegular(void) {
  ucControlTable[CH8] =
      (uint32_t)SourcePt + BlockSize - 1;  // last address of memory
  ucControlTable[CH8 + 1] =
      (uint32_t)DestinationPt;  // fixed address (when DSTINC is 0x3)
  ucControlTable[CH8 + 2] =
      0xC0000003 +
      ((BlockSize - 1) << 4);  // DMA Channel Control Word (DMACHCTL)
...
void static setAlternate(void) {
  ucControlTable[CH8ALT] =
      (uint32_t)SourcePt + BlockSize - 1;  // last address of memory
  ucControlTable[CH8ALT + 1] =
      (uint32_t)DestinationPt;  // fixed address (when DSTINC is 0x3)
  ucControlTable[CH8ALT + 2] =
      0xC0000003 +
      ((BlockSize - 1) << 4);  // DMA Channel Control Word (DMACHCTL)
                               /* DMACHCTL          Bits    Value Description
...
//          blocksize is the size of each block (max is 2048 bytes) (must be
//          even) numblocks is the number of blocks
// Outputs: none
// This routine does not wait for completion
void DMA_Transfer(uint8_t *source, uint32_t blocksize, uint32_t numblocks) {
  DisableInterrupts();  // some or all of this initialization is a critical
                        // section
  Status = BUSY;
  ...
  UDMA_ENASET_R |= BIT8;       // µDMA Channel 8 is enabled
  EnableInterrupts();  // re-enable interrupts and be ready for ping-pong
                       // interrupt
}
...
void Timer5A_Handler(void) {  // interrupts after each block is transferred
                                                  ...
                                                  /*  NumberOfBuffersSent++;
                                                    if(NumberOfBuffersSent < NumBlocks){
                                                      if((ucControlTable[CH8+2]&0x0007)==0){     // regular buffer complete
                                                        setRegular();                            // rebuild channel control
                                                    structure
                                                      }
                                                      if((ucControlTable[CH8ALT+2]&0x0007)==0){  // Alternate buffer complete
                                                        setAlternate();                          // rebuild channel control
                                                    structure
                                                      }
                                                    }else{
                                                      DMA_Stop();
  ...
  if ((ucControlTable[CH8ALT + 2] & 0x0007) ==
      0) {                                        // alternate buffer complete
    NumberOfBuffersSent++;                        // increment counter
    if (NumberOfBuffersSent < (NumBlocks - 1)) {  // still more data to send
  ...
  if (((ucControlTable[CH8 + 2] & 0x0007) == 0) &&
      ((ucControlTable[CH8ALT + 2] & 0x0007) ==
       0)) {  // both buffers complete and not re-enabled above
    DMA_Stop();
  }
}
...
enum ch8status DMA_Status(void) { return Status; }
