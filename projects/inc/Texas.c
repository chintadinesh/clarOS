...
// Initialize the UART for 115,200 baud rate (assuming busfrequency system
// clock), 8 bit word length, no parity bits, one stop bit, FIFOs enabled Input:
// none
//        bus clock frequency in Hz
//        baud rate in bps
// Output: none
void UART_Init(uint32_t busfrequency, uint32_t baud) {
  ...
  //  UART0_IBRD_R = 43;                    // IBRD = int(80,000,000 / (16 *
  //  115200)) = int(43.402778) UART0_FBRD_R = 26;                    // FBRD =
  //  round(0.402778 * 64) = 26
...
// toggle bit 0
void TExaS_Task0(void) { LogicData ^= 0x01; }
// toggle bit 1
void TExaS_Task1(void) { LogicData ^= 0x02; }
// toggle bit 2
void TExaS_Task2(void) { LogicData ^= 0x04; }
// toggle bit 3
void TExaS_Task3(void) { LogicData ^= 0x08; }
// toggle bit 4
void TExaS_Task4(void) {}
// toggle bit 5
void TExaS_Task5(void) { LogicData ^= 0x20; }
// toggle bit 6
void TExaS_Task6(void) { LogicData ^= 0x40; }
// set bit 0
void TExaS_Set0(void) { LogicData |= 0x01; }
// set bit 1
void TExaS_Set1(void) { LogicData |= 0x02; }
// set bit 2
void TExaS_Set2(void) { LogicData |= 0x04; }
// set bit 3
void TExaS_Set3(void) { LogicData |= 0x08; }
// set bit 4
void TExaS_Set4(void) {}
// set bit 5
void TExaS_Set5(void) { LogicData |= 0x20; }
// set bit 6
void TExaS_Set6(void) { LogicData |= 0x40; }
// clear bit 0
void TExaS_Clear0(void) { LogicData |= 0x01; }
// clear bit 1
void TExaS_Clear1(void) { LogicData |= 0x02; }
// clear bit 2
void TExaS_Clear2(void) { LogicData |= 0x04; }
// clear bit 3
void TExaS_Clear3(void) { LogicData |= 0x08; }
// clear bit 4
void TExaS_Clear4(void) {}
// clear bit 5
void TExaS_Clear5(void) { LogicData |= 0x20; }
// clear bit 6
void TExaS_Clear6(void) { LogicData |= 0x40; }
...
void TExaS_Init(enum TExaSmode mode, uint32_t busfrequency) {
  ...
  // logic analyzer will 10 kHz output to serial port (pack 4 bits): 8 bit bit7
  // set then LA, but 7 ASCII
  ...
  if (mode == LOGICANALYZER) {
    ...
    PeriodicTask2_Init(&LogicAnalyzer, busfrequency, 10000,
                       5);  // run logic analyzer
  }
  if (mode == SCOPE) {
...
// Inputs:  none
// Outputs: none
void TExaS_Stop(void) { PeriodicTask2_Stop(); }
