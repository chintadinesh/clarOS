...
void I2C2_Init(uint32_t I2Cfreq, uint32_t busFreq) {
  ...
  //  I2C2_MCR_R = I2C_MCR_MFE|I2C_MCR_GFE;             // 9) master function
  //  enable, glitch I2C2_MCR2_R = I2C_MCR2_GFPW_4; // 4 clock glitch
...
int I2C2_Send4(uint8_t slaveAddr, uint8_t data1, uint8_t data2, uint8_t data3,
               uint8_t data4) {
  while (I2C2_MCS_R & I2C_MCS_BUSY) {
  };                              // wait for I2C ready
  I2C2_MSA_R = (slaveAddr << 1);  // MSA[7:1] is slave address
...
int I2C2_Recv(uint8_t slaveAddr, uint8_t *pData, uint32_t count) {
  ...
  switch (count) {
    case 1:
      I2C2_MSA_R =
          (slaveAddr << 1) & I2C_MSA_SA_M;  // MSA[7:1] is slave address
    ...
    case 2:
      ...
      pData[1] = (I2C2_MDR_R &
                  I2C_MDR_DATA_M);  // least significant byte // repeat if error
      break;
    default:
      I2C2_MSA_R =
          (slaveAddr << 1) & I2C_MSA_SA_M;  // MSA[7:1] is slave address
...
uint16_t I2C2_Recv2(int8_t slave) {
  ...
  do {
    ...
    I2C2_MSA_R = (slave << 1) & 0xFE;  // MSA[7:1] is slave address
    I2C2_MSA_R |= 0x01;                // MSA[0] is 1 for receive
    I2C2_MCS_R = (0 | I2C_MCS_ACK      // positive data ack
                                       //  & ~I2C_MCS_STOP    // no stop
                  | I2C_MCS_START      // generate start/restart
                  | I2C_MCS_RUN);      // master enable
    while (I2C2_MCS_R & I2C_MCS_BUSY) {
