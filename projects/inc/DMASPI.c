...
void static setRegular(void) {
  ...
  ucControlTable[CH8 + 1] =
      (uint32_t)DestinationPt;  // last address (when DSTINC is 0x3)
  ucControlTable[CH8 + 2] =
      0xD5000003 + ((Count - 1) << 4);  // DMA Channel Control Word (DMACHCTL)
...
// The source address is incremented by two each 16-bit transfer, destination
// address is fixed Inputs:  source is a pointer to a RAM buffer containing
// waveform to output
//          destination is a pointer to 32-bit DAC device (SSI0_DR_R), least
//          significant 16 bits count is the number of halfwords to transfer
//          (max is 1024 halfwords)
// Outputs: none
// This routine does not wait for completion, runs continuously
void DMA_Start(uint16_t *source, volatile uint32_t *destination,
               uint32_t count) {
  ...
  // bits 2:0 ucControlTable[CH8ALT+2] become clear when alternate structure
  // done
}
