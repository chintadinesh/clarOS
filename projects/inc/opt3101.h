 ...
 * This file originally comes from the TIDA-010021 Firmware (tidcf48.zip)
and<br>
 * was modified by Pololu to support the MSP432P401R. . Modified again for
TM4C123
 ...
 * @param  none
 * @return true if we have a new distance measurement ready to be read, false
 * otherwise.
 ...
 * This function will set one of the three entries in the array, depending on
 * which channel was measured. distances will have values in mm, and amplitudes
 * will have amplitude values. Busy wait utilization<br> 1) Start using
 * OPT3101_StartMeasurementChannel(channel);<br> 2) Wait for complete using
 * OPT3101_CheckDistanceSensor();<br> 3) Get results using
 * OPT3101_GetMeasurement();<br>
 ...
 * The ISR will set one of the three entries in the array, depending on which
 * channel was measured. distances will have values in mm, and amplitudes will
 * have amplitude values. It will also update the channel parameter Interrupt
 * utilization<br> 1) Start using OPT3101_StartMeasurementChannel(channel);<br>
 ...
 * @brief  get measurements from last measurement
 */
void OPT3101_ArmInterrupts(uint32_t *pTxChan, uint32_t distances[3],
