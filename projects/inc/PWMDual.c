...
void PWM0Dual_Init(uint16_t period) {
  ...
  // PWM0, Generator A (PWM0/PB6) goes to 0 when count==reload and 1 when
  // count==0
  PWM0_0_GENA_R = (PWM_0_GENA_ACTLOAD_ZERO | PWM_0_GENA_ACTZERO_ONE);
  // PWM0, Generator B (PWM1/PB7) goes to 0 when count==CMPA counting down and 1
  // when count==CMPA counting up
  PWM0_0_GENB_R = (PWM_0_GENB_ACTCMPAD_ZERO | PWM_0_GENB_ACTCMPAU_ONE);
  PWM0_0_LOAD_R = (period - 1) / 2;  // 5) count from zero to this number and
                                     // back to zero in (period - 1) cycles
