      ...
      if (pState->ui16Cmd == CMD_PP) {
        ...
        if (pState->ui32WriteCount > 1) {
          ...
          uDMAChannelTransferSet(
              pState->ui32TxChannel, UDMA_MODE_BASIC, pState->pui8Buffer,
              (void *)(pState->ui32Base + SSI_O_DR),
              (pState->ui32WriteCount > 1024) ? 1024
  ...
  if (ui32Data & SSI_MIS_DMARXMIS) {
    ...
    if (pState->ui32ReadCount >= 1024) {
      ...
      if (pState->ui32ReadCount != 0) {
        ...
        //
        uDMAChannelTransferSet(
            pState->ui32RxChannel, UDMA_MODE_BASIC,
            (void *)(pState->ui32Base + SSI_O_DR), pState->pui8Buffer,
            (pState->ui32ReadCount >= 1024) ? 1024 : pState->ui32ReadCount);
        ...
        // interrupt.
        //
        if ((pState->ui32ReadCount <= 1024) && (pState->ui32WriteCount > 1)) {
          HWREG(pState->ui32Base + SSI_O_ICR) = SSI_ICR_DMATXIC;
  ...
  while (1) {
    ...
    switch (pState->ui16State) {
      ...
      case STATE_CMD: {
        ...
        if (MAP_SSIDataPutNonBlocking(pState->ui32Base, pState->ui16Cmd) == 0) {
          //
      ...
      case STATE_READ_DATA_SETUP: {
        ...
        if (pState->ui16Cmd == CMD_DREAD) {
          ...
          //
          MAP_SSIAdvModeSet(pState->ui32Base, SSI_ADV_MODE_QUAD_READ);
        } else {
          //
          // Advanced read/write mode is used for the read and fast
          // read commands.
          //
          MAP_SSIAdvModeSet(pState->ui32Base, SSI_ADV_MODE_READ_WRITE);
        }
        ...
        //
          ...
          uDMAChannelControlSet(
              pState->ui32TxChannel,
              UDMA_SRC_INC_NONE | UDMA_DST_INC_NONE | UDMA_SIZE_8 | UDMA_ARB_2);
          uDMAChannelControlSet(
              pState->ui32RxChannel,
              UDMA_SRC_INC_NONE | UDMA_DST_INC_8 | UDMA_SIZE_8 | UDMA_ARB_4);
      ...
      case STATE_READ_DATA_DMA: {
        ...
        //
        HWREG(pState->ui32Base + SSI_O_IM) = SSI_IM_DMARXIM | SSI_IM_TXIM;
      ...
      case STATE_READ_DATA_END: {
        ...
        if (pState->ui32WriteCount != 0) {
          ...
          if (MAP_SSIAdvDataPutFrameEndNonBlocking(pState->ui32Base, 0) == 0) {
            //
            // The dummy byte could not be written, so return
            // indicating that the transfer is still in progress.
            //
      ...
      case STATE_WRITE_DATA_SETUP: {
        ...
        //
          ...
          uDMAChannelControlSet(
              pState->ui32TxChannel,
              UDMA_SRC_INC_8 | UDMA_DST_INC_NONE | UDMA_SIZE_8 | UDMA_ARB_4);
...
//
//*****************************************************************************
void SPIFlashPageProgramNonBlocking(tSPIFlashState *pState, uint32_t ui32Base,
...
//
//*****************************************************************************
void SPIFlashRead(uint32_t ui32Base, uint32_t ui32Addr, uint8_t *pui8Data,
                  uint32_t ui32Count) {
  uint32_t ui32Trash;
...
//
//*****************************************************************************
void SPIFlashFastRead(uint32_t ui32Base, uint32_t ui32Addr, uint8_t *pui8Data,
                      uint32_t ui32Count) {
  uint32_t ui32Trash;
...
//
//*****************************************************************************
void SPIFlashFastReadNonBlocking(tSPIFlashState *pState, uint32_t ui32Base,
                                 uint32_t ui32Addr, uint8_t *pui8Data,
                                 uint32_t ui32Count, bool bUseDMA,
                                 uint32_t ui32TxChannel,
...
//
//*****************************************************************************
void SPIFlashDualRead(uint32_t ui32Base, uint32_t ui32Addr, uint8_t *pui8Data,
                      uint32_t ui32Count) {
  uint32_t ui32Trash;
...
//
//*****************************************************************************
void SPIFlashDualReadNonBlocking(tSPIFlashState *pState, uint32_t ui32Base,
                                 uint32_t ui32Addr, uint8_t *pui8Data,
                                 uint32_t ui32Count, bool bUseDMA,
                                 uint32_t ui32TxChannel,
                                 uint32_t ui32RxChannel) {
...
//
//*****************************************************************************
void SPIFlashQuadRead(uint32_t ui32Base, uint32_t ui32Addr, uint8_t *pui8Data,
                      uint32_t ui32Count) {
  uint32_t ui32Trash;
...
//
//*****************************************************************************
void SPIFlashQuadReadNonBlocking(tSPIFlashState *pState, uint32_t ui32Base,
                                 uint32_t ui32Addr, uint8_t *pui8Data,
                                 uint32_t ui32Count, bool bUseDMA,
                                 uint32_t ui32TxChannel,
                                 uint32_t ui32RxChannel) {
