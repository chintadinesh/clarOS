...
//
//*****************************************************************************
#define TX_BUFFER_USED                                             \
  (GetBufferCount(&g_ui32UARTTxReadIndex, &g_ui32UARTTxWriteIndex, \
                  UART_TX_BUFFER_SIZE))
#define TX_BUFFER_FREE (UART_TX_BUFFER_SIZE - TX_BUFFER_USED)
#define TX_BUFFER_EMPTY \
  (IsBufferEmpty(&g_ui32UARTTxReadIndex, &g_ui32UARTTxWriteIndex))
#define TX_BUFFER_FULL                                           \
  (IsBufferFull(&g_ui32UARTTxReadIndex, &g_ui32UARTTxWriteIndex, \
                UART_TX_BUFFER_SIZE))
...
//
//*****************************************************************************
#define RX_BUFFER_USED                                             \
  (GetBufferCount(&g_ui32UARTRxReadIndex, &g_ui32UARTRxWriteIndex, \
                  UART_RX_BUFFER_SIZE))
#define RX_BUFFER_FREE (UART_RX_BUFFER_SIZE - RX_BUFFER_USED)
#define RX_BUFFER_EMPTY \
  (IsBufferEmpty(&g_ui32UARTRxReadIndex, &g_ui32UARTRxWriteIndex))
#define RX_BUFFER_FULL                                           \
  (IsBufferFull(&g_ui32UARTRxReadIndex, &g_ui32UARTRxWriteIndex, \
                UART_RX_BUFFER_SIZE))
...
//
//*****************************************************************************
static const uint32_t g_ui32UARTBase[3] = {UART0_BASE, UART1_BASE, UART2_BASE};
...
//
//*****************************************************************************
static const uint32_t g_ui32UARTInt[3] = {INT_UART0, INT_UART1, INT_UART2};
...
void UARTStdioConfig(uint32_t ui32PortNum, uint32_t ui32Baud,
  ...
  MAP_UARTConfigSetExpClk(
      g_ui32Base, ui32SrcClock, ui32Baud,
...
//!
//! \param pcBuf points to null-terminated string containing the data to
//! transmit.
//!
...
//!
//! In non-buffered mode, this function is blocking and will not return until
//! all the characters have been written to the output FIFO.  Buffered mode is
//! not implemented,
...
void UARTOutString(char *pcBuf) {
  ...
  while (*pcBuf) {         // Send the characters
    ...
    MAP_UARTCharPut(g_ui32Base,
                    *pcBuf);  // Send the character to the UART output.
    pcBuf++;
  }
  //#endif
}
    ...
    again:
      ...
      switch (*pcString++) {
        ...
        case 'p': {
          ...
          //
        convert:
          for (ui32Idx = 1; (((ui32Idx * ui32Base) <= ui32Value) &&
