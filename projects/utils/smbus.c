...
void SMBusARPUDIDPacketEncode(tSMBusUDID *pUDID, uint8_t ui8Address,
  ...
  pui8Data[12] = (uint8_t)((pUDID->ui32VendorSpecificID & 0xff000000) >> 24);
  pui8Data[13] = (uint8_t)((pUDID->ui32VendorSpecificID & 0x00ff0000) >> 16);
  pui8Data[14] = (uint8_t)((pUDID->ui32VendorSpecificID & 0x0000ff00) >> 8);
...
//
//*****************************************************************************
void SMBusARPUDIDPacketDecode(tSMBusUDID *pUDID, uint8_t *pui8Address,
                              uint8_t *pui8Data) {
  //
  ...
  pUDID->ui16SubSystemVendorID = (uint16_t)((pui8Data[8] << 8) | pui8Data[9]);
  pUDID->ui16SubSystemDeviceID = (uint16_t)((pui8Data[10] << 8) | pui8Data[11]);
  pUDID->ui32VendorSpecificID =
      (uint32_t)((pui8Data[12] << 24) | (pui8Data[13] << 16) |
                 (pui8Data[14] << 8) | pui8Data[15]);
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterQuickCommand(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                     bool bData) {
  //
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterHostNotify(tSMBus *psSMBus, uint8_t ui8OwnSlaveAddress,
                                   uint8_t *pui8Data) {
  //
  // Make sure that the peripheral is not currently active.
  //
  ...
  // Initiate the write operation.
  //
  MAP_I2CMasterControl(psSMBus->ui32I2CBase, I2C_MASTER_CMD_BURST_SEND_START);
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterByteSend(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                 uint8_t ui8Data) {
  uint8_t ui8TempData;
  ...
  if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
    ...
    psSMBus->ui8CalculatedCRC =
        MAP_Crc8CCITT(psSMBus->ui8CalculatedCRC, &psSMBus->pui8TxBuffer[0], 1);
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterByteReceive(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                    uint8_t *pui8Data) {
  uint8_t ui8TempData;
  ...
  if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
    ...
    // Initiate the read operation.
    //
    MAP_I2CMasterControl(psSMBus->ui32I2CBase, I2C_MASTER_CMD_SINGLE_RECEIVE);
  }
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterByteWordWrite(tSMBus *psSMBus, uint8_t ui8TargetAddress,
  ...
  if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
    ...
    //
    psSMBus->ui8CalculatedCRC = MAP_Crc8CCITT(
        psSMBus->ui8CalculatedCRC, psSMBus->pui8TxBuffer, psSMBus->ui8TxSize);
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterByteWordRead(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                     uint8_t ui8Command, uint8_t *pui8Data,
                                     uint8_t ui8Size) {
  uint8_t ui8TempData;
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterBlockWrite(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                   uint8_t ui8Command, uint8_t *pui8Data,
                                   uint8_t ui8Size) {
  uint8_t ui8TempData;
  ...
  if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
    ...
    //
    psSMBus->ui8CalculatedCRC =
        MAP_Crc8CCITT(psSMBus->ui8CalculatedCRC, &psSMBus->ui8TxSize, 1);
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterBlockRead(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                  uint8_t ui8Command, uint8_t *pui8Data) {
  uint8_t ui8TempData;
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterProcessCall(tSMBus *psSMBus, uint8_t ui8TargetAddress,
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterBlockProcessCall(tSMBus *psSMBus,
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterI2CWrite(tSMBus *psSMBus, uint8_t ui8TargetAddress,
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterI2CRead(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                uint8_t *pui8Data, uint8_t ui8Size) {
  //
  // Make sure that the peripheral is not currently active.
  //
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterI2CWriteRead(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                     uint8_t *pui8TxData, uint8_t ui8TxSize,
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterARPGetUDIDGen(tSMBus *psSMBus, uint8_t *pui8Data) {
  //
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterARPGetUDIDDir(tSMBus *psSMBus, uint8_t ui8TargetAddress,
                                      uint8_t *pui8Data) {
  //
  // Use the block read protocol to receive the UDID.
  //
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterARPResetDeviceGen(tSMBus *psSMBus) {
  //
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterARPResetDeviceDir(tSMBus *psSMBus,
                                          uint8_t ui8TargetAddress) {
  //
  // Use the Send Byte protocol to send the packet.
  //
...
//
//*****************************************************************************
tSMBusStatus SMBusMasterARPAssignAddress(tSMBus *psSMBus, uint8_t *pui8Data) {
  //
...
tSMBusStatus SMBusMasterIntProcess(tSMBus *psSMBus) {
  ...
  if (ui32IntStatus & I2C_MASTER_INT_TIMEOUT) {
    ...
    //
    MAP_I2CMasterIntClearEx(psSMBus->ui32I2CBase,
  ...
  switch (psSMBus->ui8MasterState) {
    ...
    case SMBUS_STATE_WRITE_FINAL: {
      ...
      if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
        ...
        if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PROCESS_CALL)) {
          ...
          // register.
          //
          MAP_I2CMasterDataPut(psSMBus->ui32I2CBase, psSMBus->ui8CalculatedCRC);
        }
      } else {
        //
    ...
    case SMBUS_STATE_READ_BLOCK_SIZE: {
      ...
      if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
        ...
        //
        psSMBus->ui8CalculatedCRC =
            MAP_Crc8CCITT(psSMBus->ui8CalculatedCRC, &psSMBus->ui8RxSize, 1);
      }
      ...
      if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
        ...
        psSMBus->ui8CalculatedCRC =
            MAP_Crc8CCITT(psSMBus->ui8CalculatedCRC,
                          &psSMBus->pui8RxBuffer[psSMBus->ui8RxIndex], 1);
...
void SMBusMasterIntEnable(tSMBus *psSMBus) {
  ...
  //
  MAP_I2CMasterIntEnableEx(psSMBus->ui32I2CBase,
                           I2C_MASTER_INT_DATA | I2C_MASTER_INT_TIMEOUT);
...
//
//*****************************************************************************
void SMBusMasterInit(tSMBus *psSMBus, uint32_t ui32I2CBase,
    ...
    switch ((ui32SlaveStatus & 0x07)) {
      ...
      case I2C_SLAVE_ACT_RREQ_FBR: {
        ...
        //
          ...
          psSMBus->ui8CurrentCommand = I2CSlaveDataGet(psSMBus->ui32I2CBase);
        }
        ...
        if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
          ...
          //
          psSMBus->ui8CalculatedCRC = MAP_Crc8CCITT(
              psSMBus->ui8CalculatedCRC, &psSMBus->ui8CurrentCommand, 1);
        }
        ...
        switch (psSMBus->ui8SlaveState) {
          ...
          case SMBUS_STATE_SLAVE_POST_COMMAND: {
            ...
            if (HWREGBITB(&psSMBus->ui16Flags, FLAG_BLOCK_TRANSFER)) {
              ...
              if (ui8DataTemp > psSMBus->ui8RxSize) {
                ...
                if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
                  ...
                  //
                  psSMBus->ui8CalculatedCRC =
                      MAP_Crc8CCITT(psSMBus->ui8CalculatedCRC, &ui8DataTemp, 1);
                }
            ...
            if (psSMBus->ui8RxIndex == psSMBus->ui8RxSize) {
              ...
              //
              psSMBus->pui8RxBuffer[psSMBus->ui8RxIndex++] = ui8DataTemp;
              ...
              if (psSMBus->ui8RxIndex == psSMBus->ui8RxSize) {
                ...
                if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
                  ...
                  // Update the state machine.
                  //
                  psSMBus->ui8SlaveState = SMBUS_STATE_READ_PEC;
                } else {
                  //
                  // Update the state machine.
                  //
        ...
        switch (psSMBus->ui8SlaveState) {
          ...
          case SMBUS_STATE_READ_DONE: {
            ...
            if (HWREGBITB(&psSMBus->ui16Flags, FLAG_PEC)) {
              ...
              //
              psSMBus->ui8CalculatedCRC =
                  MAP_Crc8CCITT(psSMBus->ui8CalculatedCRC, &ui8CRCTemp, 1);
...
//
//*****************************************************************************
void SMBusSlaveTxBufferSet(tSMBus *psSMBus, uint8_t *pui8Data,
                           uint8_t ui8Size) {
  //
...
//
//*****************************************************************************
void SMBusSlaveRxBufferSet(tSMBus *psSMBus, uint8_t *pui8Data,
                           uint8_t ui8Size) {
  //
...
//
//*****************************************************************************
void SMBusSlaveAddressSet(tSMBus *psSMBus, uint8_t ui8AddressNum,
