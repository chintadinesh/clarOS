...
//
//*****************************************************************************
void SoftI2CTimerTick(tSoftI2C *psI2C) {
  //
  ...
  switch (psI2C->ui8State) {
    ...
    case SOFTI2C_STATE_ADDR0: {
      ...
      if (psI2C->ui8CurrentBit < 7) {
        ...
        HWREG(psI2C->ui32SDAGPIO) =
            ((psI2C->ui8SlaveAddr & (1 << (6 - psI2C->ui8CurrentBit))) ? 255
                                                                       : 0);
      }
      ...
      //
        ...
        //
        HWREG(psI2C->ui32SDAGPIO) =
            (HWREGBITB(&(psI2C->ui8Flags), SOFTI2C_FLAG_RECEIVE) ? 255 : 0);
      }
    ...
    case SOFTI2C_STATE_SEND0: {
      ...
      if (psI2C->ui8CurrentBit < 8) {
        ...
        //
        HWREG(psI2C->ui32SDAGPIO) =
            ((psI2C->ui8Data & (1 << (7 - psI2C->ui8CurrentBit))) ? 255 : 0);
      }
    ...
    case SOFTI2C_STATE_RECV3: {
      ...
      //
        ...
        //
        psI2C->ui8Data |=
            (HWREG(psI2C->ui32SDAGPIO) ? (1 << (7 - psI2C->ui8CurrentBit)) : 0);
...
//
//*****************************************************************************
void SoftI2CInit(tSoftI2C *psI2C) {
  //
  // Configure the SCL pin.
  //
  MAP_GPIODirModeSet(psI2C->ui32SCLGPIO & 0xfffff000,
                     (psI2C->ui32SCLGPIO & 0x00000fff) >> 2, GPIO_DIR_MODE_OUT);
  ...
  //
  MAP_GPIODirModeSet(psI2C->ui32SDAGPIO & 0xfffff000,
                     (psI2C->ui32SDAGPIO & 0x00000fff) >> 2, GPIO_DIR_MODE_OUT);
...
//
//*****************************************************************************
void SoftI2CCallbackSet(tSoftI2C *psI2C, void (*pfnCallback)(void)) {
  //
...
//
//*****************************************************************************
void SoftI2CSCLGPIOSet(tSoftI2C *psI2C, uint32_t ui32Base, uint8_t ui8Pin) {
  //
...
//
//*****************************************************************************
void SoftI2CSDAGPIOSet(tSoftI2C *psI2C, uint32_t ui32Base, uint8_t ui8Pin) {
  //
...
//
//*****************************************************************************
void SoftI2CIntEnable(tSoftI2C *psI2C) {
  //
...
//
//*****************************************************************************
void SoftI2CIntDisable(tSoftI2C *psI2C) {
  //
...
//
//*****************************************************************************
bool SoftI2CIntStatus(tSoftI2C *psI2C, bool bMasked) {
  //
...
//
//*****************************************************************************
void SoftI2CIntClear(tSoftI2C *psI2C) {
  //
...
//
//*****************************************************************************
void SoftI2CSlaveAddrSet(tSoftI2C *psI2C, uint8_t ui8SlaveAddr, bool bReceive) {
  //
  // Check the arguments.
  //
...
//
//*****************************************************************************
bool SoftI2CBusy(tSoftI2C *psI2C) {
  //
...
//
//*****************************************************************************
void SoftI2CControl(tSoftI2C *psI2C, uint32_t ui32Cmd) {
  //
  // Check the arguments.
  //
...
//
//*****************************************************************************
uint32_t SoftI2CErr(tSoftI2C *psI2C) {
  //
...
//
//*****************************************************************************
void SoftI2CDataPut(tSoftI2C *psI2C, uint8_t ui8Data) {
  //
  // Write the byte.
  //
...
//
//*****************************************************************************
uint32_t SoftI2CDataGet(tSoftI2C *psI2C) {
  //
  // Read a byte.
  //
