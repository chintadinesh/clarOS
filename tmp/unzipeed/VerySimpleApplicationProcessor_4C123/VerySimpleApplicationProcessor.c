...
#include "../inc/UART.h"
//--------------------------------------------------
// 1. Initialize GATT (add services, characteristics, CCCD’s, etc.). See
// Section 9.5.
// 2. Initialize GAP (advertisement data, connection parameters, etc.). See
// Section 9.3.
...
// first characteristic is called "Data" and is a read/write parameter exchanged
// with the phone
uint8_t Data;      // read/write parameter
uint16_t Handle1;  // number associated with this characteristic
// second characteristic is called "Switches" and is a read only parameter that
// returns switch value (0-3)
uint16_t Handle2;  // number associated with this characteristic
// third characteristic is called "LEDs" and is a write-only parameter that sets
// the LED (0-7)
uint16_t Handle3;  // number associated with this characteristic
// fourth characteristic is called "Count" and is a local counter sent
// periodically to the phone
uint8_t Count;
uint16_t Handle4;     // number associated with this characteristic
uint16_t CCCDhandle;  // number associated with this CCCD, Client Characteristic
...
const uint8_t NPI_GATTSetDeviceNameMsg[] = {
    ...
    'S',  'h',  'a',  'p', 'e', ' ', 't', 'h',
    'e',  ' ',  'W',  'o', 'r', 'l', 'd', 0x77};  // FCS (calculated by
...
const uint8_t NPI_AddCharDescriptor2[] = {
    ...
    'S',  'w',  'i',  't', 'c',
    'h',  'e',  's',  0,  // Initial user description string
...
int main(void) {
  ...
  r1 = AP_SendMessageResponse((uint8_t *)NPI_GATTSetDeviceNameMsg, RecvBuf,
                              RECVSIZE);
  ...
  r1 = AP_SendMessageResponse((uint8_t *)NPI_AddCharDescriptor1, RecvBuf,
                              RECVSIZE);
  ...
  r1 = AP_SendMessageResponse((uint8_t *)NPI_AddCharDescriptor2, RecvBuf,
                              RECVSIZE);
  ...
  r1 = AP_SendMessageResponse((uint8_t *)NPI_AddCharDescriptor3, RecvBuf,
                              RECVSIZE);
  ...
  r1 = AP_SendMessageResponse((uint8_t *)NPI_AddCharDescriptor4, RecvBuf,
                              RECVSIZE);
  ...
  //--------------Advertise----------
  UART_OutString("\n\rSetAdvertisement1");
  r1 = AP_SendMessageResponse((uint8_t *)NPI_SetAdvertisementMsg, RecvBuf,
                              RECVSIZE);
  UART_OutString("\n\rSetAdvertisement2");
  r1 = AP_SendMessageResponse((uint8_t *)NPI_SetAdvertisementDataMsg, RecvBuf,
                              RECVSIZE);
  UART_OutString("\n\rStartAdvertisement");
  r1 = AP_SendMessageResponse((uint8_t *)NPI_StartAdvertisementMsg, RecvBuf,
                              RECVSIZE);
  UART_OutString("\n\rGetStatus");
  r1 = AP_SendMessageResponse((uint8_t *)NPI_GetStatusMsg, RecvBuf, RECVSIZE);
  ...
  while (1) {
    ...
    if (AP_RecvStatus()) {
      if (AP_RecvMessage(RecvBuf, RECVSIZE) == APOK) {
        ...
        if ((RecvBuf[3] == 0x55) &&
            (RecvBuf[4] ==
             0x88)) {  // SNP Characteristic Write Indication (0x88)
        ...
        if ((RecvBuf[3] == 0x55) &&
            (RecvBuf[4] ==
             0x87)) {  // SNP Characteristic Read Indication (0x87)
          h = (RecvBuf[8] << 8) + RecvBuf[7];  // handle for this characteristic
    ...
    if (time > 4000000) {
      ...
      if (CCCDvalue) {
        ...
        OutValue("\n\rSend Count=", Count);
        r1 = AP_SendMessageResponse(NPI_SendNotificationIndicationMsg, RecvBuf,
                                    RECVSIZE);
      }
    }
  }
