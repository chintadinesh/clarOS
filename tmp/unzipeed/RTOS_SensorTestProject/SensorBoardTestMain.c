...
// 0) Set the OPT3101 compiler flag to 1 if using the OPT3101 (set OPT3101
// compiler flag to 0 if not using) 1) Connect the LaunchPad to the PC using the
// USB cable.  Open, build (compile), and download SensorBoardTest.uvproj in
// uVision4. 2) Disconnect the LaunchPad's USB cable from the PC. 3) If the
// Sensor Board power (J13) is still connected to the Motor Board power, do one
// of the following:
//      a) Remove JP1 to power the Sensor Board from the PC.  Connect the
//      LaunchPad to the Sensor Board. b) Insert JP1 to power the Sensor Board
//      from the Motor Board's power.  Be sure that the Motor Board is off, and
//      then connect the LaunchPad to the Sensor Board.  The LaunchPad's USB
//      cable can be removed.
// 4) Connect the ST7735 LCD to the Sensor Board.
// 5) Connect all necessary sensors to the Sensor Board.  There is nothing
// stopping you from connecting the ultrasonic sensors backwards or off-by-one.
// Be sure to match the ground wire with the "G" label on the Sensor Board. Also
// be sure to match the same number of wires in the plug as in the socket.  The
// 3-wire and 4-wire connectors are different.
//      NOTE: Only 3 of the 4 ultrasonic sensors may be used.  J12X and J12Y
//      share PF4 with a built-in switch on the LaunchPad. NOTE: If using the
//      OPT3101 distance sensor, you can not use ultrasonics on J11 or J12
// 6) Power up the Sensor Board by doing one of the following:
//      a) Re-connect the LaunchPad's USB cable to the PC.
//      b) Turn on the power to the Motor Board.  Depending on how the rest of
//      the robot is configured, the wheels may spin, so hold it or prop it up
//      so it does not get away.
// 7) Follow the onscreen instructions.  Press SW1 if using 4-pin HC-SR04
// ultrasonic sensors or press SW2 if using 3-pin Ping))) ultrasonic sensors. 8)
// Manipulate the sensors and verify that the displayed numbers change.  The
// numbers should also be steady when stationary.  Remember that the sensors may
// have a "blind spot" at extremely close range.
...
int main(void) {
  ...
  while (((LaunchPad_Input() & SW2) == 0) &&
         ((LaunchPad_Input() & SW1) == 0)) {  // advance when SW1 or SW2 pressed
    Clock_Delay1ms(10);
  }
  ...
  while (1) {
    ...
    if ((time & 0x7F) == 0) {  // output every 128th loop (128ms)
      ...
      ST7735_OutUDec4(FilteredDistances[0]);
      ST7735_SetCursor(11, 14);
      ST7735_OutUDec4(FilteredDistances[1]);
      ST7735_SetCursor(11, 15);
      ST7735_OutUDec4(FilteredDistances[2]);
#else
      ST7735_SetCursor(11, 9);
      Cycle0 = Timer0_Read();
      if (Cycle0) {
