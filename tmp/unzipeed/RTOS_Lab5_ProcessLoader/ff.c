...
/                   Improved performance of f_lseek() on moving to the same or
following cluster.
/
...
/                   Fixed f_unlink() fails to remove a sub-directory on
_FS_RPATH. /                   Fixed name matching error on the 13 character
boundary. /                   Added a configuration option, _LFN_UNICODE. /
...
/                   String functions support UTF-8 encoding files on Unicode
cfg. / Aug 16,'10 R0.08a Added f_getcwd(). /                   Added sector
erase feature. (_USE_ERASE) /                   Moved file lock semaphore table
from fs object to the bss. /                   Fixed a wrong directory entry is
created on non-LFN cfg when the given name contains ';'. / Fixed f_mkfs()
creates wrong FAT32 volume. / Jan 15,'11 R0.08b Fast seek feature is also
applied to f_read() and f_write(). /                   f_lseek() reports
required table size on creating CLMP. /                   Extended format syntax
of f_printf(). /                   Ignores duplicated directory separators in
given path name.
/
/ Sep 06,'11 R0.09  f_mkfs() supports multiple partition to complete the
multiple partition feature. /                   Added f_fdisk(). / Aug 27,'12
R0.09a Changed f_open() and f_opendir() reject null object pointer to avoid
crash. /                   Changed option name _FS_SHARE to _FS_LOCK. / Fixed
assertion failure due to OS/2 EA on FAT12/16 volume. / Jan 24,'13 R0.09b Added
f_setlabel() and f_getlabel().
/
/ Oct 02,'13 R0.10  Added selection of character encoding on the file.
(_STRF_ENCODE) /                   Added f_closedir(). /                   Added
forced full FAT scan for f_getfree(). (_FS_NOFSINFO) /                   Added
forced mount feature with changes of f_mount(). /                   Improved
behavior of volume auto detection. /                   Improved write throughput
of f_puts() and f_printf(). /                   Changed argument of f_chdrive(),
f_mkfs(), eDisk_Read() and eDisk_Write(). /                   Fixed f_write()
can be truncated when the file size is close to 4GB. /                   Fixed
f_open(), f_mkdir() and f_setlabel() can return incorrect error code. / Jan
15,'14 R0.10a Added arbitrary strings as drive number in the path name.
(_STR_VOLUME_ID) /                   Added a configuration option of minimum
sector size. (_MIN_SS) /                   2nd argument of f_rename() can have a
drive number and it will be ignored. /                   Fixed f_mount() with
forced mount fails when drive number is >= 1. /                   Fixed
f_close() invalidates the file object without volume lock. / Fixed f_closedir()
returns but the volume lock is left acquired. /                   Fixed creation
of an entry with LFN fails on too many SFN collisions. / May 19,'14 R0.10b Fixed
a hard error in the disk I/O layer can collapse the directory entry. / Fixed LFN
entry is not deleted on delete/rename an object with lossy converted SFN. / Nov
09,'14 R0.10c Added a configuration option for the platforms without RTC.
(_FS_NORTC) /                   Fixed volume label created by Mac OS X cannot be
retrieved with f_getlabel(). /                   Fixed a potential problem of
FAT access that can appear on disk error. /                   Fixed null pointer
dereference on attempting to delete the root direcotry. / Patch1 applied. /
Patch2 applied.
/---------------------------------------------------------------------------*/
...
#if (_MAX_SS < _MIN_SS) ||                                   \
    (_MAX_SS != 512 && _MAX_SS != 1024 && _MAX_SS != 2048 && \
     _MAX_SS != 4096) ||                                     \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x9A, 0x90, 0x41, 0x8E, 0x41, 0x8F, 0x80, 0x45, 0x45, 0x45, 0x49, \
        ...
        0x21, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x45, 0x41, 0x84, 0x41, 0x86, 0x43, 0x45, 0x45, 0x45, 0x49, \
        0x49, 0x8D, 0x8E, 0x8F, 0x90, 0x92, 0x92, 0x93, 0x94, 0x95, 0x49,   \
        0x49, 0x98, 0x99, 0x9A, 0x9B, 0x9C, 0x9D, 0x9E, 0x9F, 0xA0, 0xA1,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        ...
        0xE4, 0xED, 0xEE, 0xE7, 0xE8, 0xF1, 0xEA, 0xEB, 0xEC, 0xED, 0xEE,   \
        0xEF, 0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9,   \
        0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF                                  \
  }
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x9A, 0x90, 0xB6, 0x8E, 0xB7, 0x8F, 0x80, 0xD2, 0xD3, 0xD4, 0xD8, \
        0xD7, 0xDE, 0x8E, 0x8F, 0x90, 0x92, 0x92, 0xE2, 0x99, 0xE3, 0xEA,   \
        0xEB, 0x59, 0x99, 0x9A, 0x9D, 0x9C, 0x9D, 0x9E, 0x9F, 0xB5, 0xD6,   \
        0xE0, 0xE9, 0xA5, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0x21, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,   \
        0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC7, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
        0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xE0, 0xE1, 0xE2, 0xE3,   \
        0xE5, 0xE5, 0xE6, 0xE7, 0xE7, 0xE9, 0xEA, 0xEB, 0xED, 0xED, 0xEE,   \
        0xEF, 0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9,   \
        0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF                                  \
  }
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x9A, 0x90, 0xB6, 0x8E, 0xDE, 0x8F, 0x80, 0x9D, 0xD3, 0x8A, 0x8A, \
        ...
        0x97, 0x97, 0x99, 0x9A, 0x9B, 0x9B, 0x9D, 0x9E, 0x9F, 0xB5, 0xD6,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x9A, 0x90, 0xB6, 0x8E, 0xB7, 0x8F, 0x80, 0xD2, 0xD3, 0xD4, 0xD8, \
        0xD7, 0x98, 0x8E, 0x8F, 0x90, 0x92, 0x92, 0xE2, 0x99, 0xE3, 0xEA,   \
        ...
        0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xE0, 0xE1, 0xE2, 0xE3,   \
        0xE5, 0xE5, 0xE6, 0xE7, 0xE8, 0xE9, 0xEA, 0xEB, 0xDE, 0x59, 0xEE,   \
        0xEF, 0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9,   \
        0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF                                  \
  }
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x9A, 0x90, 0xB6, 0x8E, 0xB7, 0x8F, 0x80, 0xD2, 0xD3, 0xD4, 0xD8, \
        ...
        0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC7, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
        0xCE, 0xCF, 0xD1, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xE0, 0xE1, 0xE2, 0xE3,   \
        0xE5, 0xE5, 0xE6, 0xE7, 0xE7, 0xE9, 0xEA, 0xEB, 0xED, 0xED, 0xEE,   \
        0xEF, 0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9,   \
        0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF                                  \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        ...
        0x9d, 0x95, 0x96, 0x97, 0x98, 0x99, 0x9A, 0x9B, 0x9C, 0x9D, 0x9E,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,   \
        0x97, 0x98, 0x99, 0x8A, 0x9B, 0x8C, 0x8D, 0x8E, 0x8F, 0xA0, 0xA1,   \
        0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0xAD, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xA3, 0xB4, 0xB5, 0xB6, 0xB7,   \
        0xB8, 0xA5, 0xAA, 0xBB, 0xBC, 0xBD, 0xBC, 0xAF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
        0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xC0, 0xC1, 0xC2, 0xC3,   \
        0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD, 0xCE,   \
        0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xF7, 0xD8, 0xD9,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x82, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        0x8C, 0x8D, 0x8E, 0x8F, 0x80, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,   \
        0x97, 0x98, 0x99, 0x8A, 0x9B, 0x8C, 0x8D, 0x8E, 0x8F, 0xA0, 0xA2,   \
        0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0xAD, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB2, 0xA5, 0xB5, 0xB6, 0xB7,   \
        0xA8, 0xB9, 0xAA, 0xBB, 0xA3, 0xBD, 0xBD, 0xAF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
        0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xC0, 0xC1, 0xC2, 0xC3,   \
        0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD, 0xCE,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,   \
        0x97, 0x98, 0x99, 0xAd, 0x9B, 0x8C, 0x9D, 0xAE, 0x9F, 0xA0, 0x21,   \
        0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0xAD, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,   \
        0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        ...
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
        0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xA2, 0xB8, 0xB9, 0xBA, 0xE0, 0xC1, 0xC2, 0xC3,   \
        0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD, 0xCE,   \
        0xCF, 0xD0, 0xD1, 0xF2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8, 0xD9,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,   \
        0x97, 0x98, 0x99, 0x8A, 0x9B, 0x8C, 0x9D, 0x9E, 0x9F, 0xA0, 0x21,   \
        0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0xAD, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,   \
        0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,   \
        0x97, 0x98, 0x99, 0x9A, 0x9B, 0x9C, 0x9D, 0x9E, 0x9F, 0xA0, 0x21,   \
        0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0xAD, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,   \
        0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,   \
        0x97, 0x98, 0x99, 0x9A, 0x9B, 0x8C, 0x9D, 0x9E, 0x9F, 0xA0, 0xA1,   \
        0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0xAD, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,   \
        0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
        0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0x41, 0xE1, 0x41, 0xE3,   \
        0xE4, 0xE5, 0xE6, 0x43, 0x45, 0x45, 0x45, 0x45, 0xEC, 0xED, 0x49,   \
        0x49, 0xF0, 0xF1, 0xF2, 0xF3, 0x4F, 0xF5, 0xF6, 0xF7, 0xF8, 0x55,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        ...
        0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0xAD, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,   \
        0xA8, 0xB9, 0xAA, 0xBB, 0xBC, 0xBD, 0xBE, 0xAF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
        0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xC0, 0xC1, 0xC2, 0xC3,   \
        0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD, 0xCE,   \
...
#define _EXCVT                                                              \
  {                                                                         \
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, \
        0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,   \
        0x97, 0x98, 0x99, 0x9A, 0x9B, 0xAC, 0x9D, 0x9E, 0x9F, 0xA0, 0x21,   \
        0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,   \
        0xAD, 0xAE, 0xAF, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,   \
        0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0xC2,   \
        0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,   \
        0xCE, 0xCF, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8,   \
        0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xC0, 0xC1, 0xC2, 0xC3,   \
        0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xEC, 0xCD, 0xCE,   \
        0xCF, 0xD0, 0xD1, 0xF2, 0xD3, 0xD4, 0xD5, 0xD6, 0xF7, 0xD8, 0xD9,   \
...
chk_lock(         /* Check if the file can be accessed */
  ...
  for (i = be = 0; i < _FS_LOCK; i++) {
    if (Files[i].fs) { /* Existing entry */
      if (Files[i].fs ==
              dp->fs && /* Check if the object matched with an open object */
...
static FRESULT sync_fs(          /* FR_OK: successful, FR_DISK_ERR: failed */
  ...
  if (res == FR_OK) {
    ...
    if (fs->fs_type == FS_FAT32 && fs->fsi_flag == 1) {
      ...
      eDisk_Write(fs->drv, fs->win, fs->winsect, 1);
      fs->fsi_flag = 0;
    }
...
DWORD get_fat(/* 0xFFFFFFFF:Disk error, 1:Internal error, 2..0x0FFFFFFF:Cluster
                 status */
              FATFS* fs, /* File system object */
...
static DWORD clmt_clust(          /* <2:Error, >=2:Cluster number */
  ...
  cl =
      ofs / SS(fp->fs) / fp->fs->csize; /* Cluster order from top of the file */
  for (;;) {
...
static FRESULT dir_sdi(DIR* dp, /* Pointer to directory object */
  ...
  if (clst == 0) { /* Static table (root-directory in FAT12/16) */
    ...
    while (idx >= ic) {                           /* Follow cluster chain */
      ...
      if (clst < 2 ||
          clst >=
              dp->fs->n_fatent) /* Reached to end of table or internal error */
        return FR_INT_ERR;
      idx -= ic;
    }
  ...
  dp->sect =
      sect + idx / (SS(dp->fs) / SZ_DIR); /* Sector# of the directory entry */
  dp->dir = dp->fs->win + (idx % (SS(dp->fs) / SZ_DIR)) *
...
dir_next(/* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:Could not
  ...
  if (!(i % (SS(dp->fs) / SZ_DIR))) { /* Sector changed? */
    ...
    if (!dp->clust) { /* Static table */
      if (i >=
          dp->fs->n_rootdir) /* Report EOT if it reached end of static table */
        return FR_NO_FILE;
    } else { /* Dynamic table */
      if (((i / (SS(dp->fs) / SZ_DIR)) & (dp->fs->csize - 1)) ==
          0) {                             /* Cluster changed? */
        clst = get_fat(dp->fs, dp->clust); /* Get next cluster */
        if (clst <= 1) return FR_INT_ERR;
        if (clst == 0xFFFFFFFF) return FR_DISK_ERR;
        if (clst >=
            dp->fs->n_fatent) { /* If it reached end of dynamic table, */
#if !_FS_READONLY
          UINT c;
...
#else
          if (!stretch)
            return FR_NO_FILE; /* If do not stretch, report EOT (this is to
                                  suppress warning) */
  ...
  dp->dir = dp->fs->win + (i % (SS(dp->fs) / SZ_DIR)) *
                              SZ_DIR; /* Current entry in the window */
  ...
  while (dp->sect) {
    ...
    if (c == DDE || (!_FS_RPATH && c == '.') ||
      ...
      if (a == AM_LFN) { /* An LFN entry is found */
        ...
        ord = (c == ord && sum == dir[LDIR_Chksum] && pick_lfn(dp->lfn, dir))
                  ? ord - 1
                  : 0xFF;
      } else {                          /* An SFN entry is found */
...
static FRESULT create_name(DIR* dp, /* Pointer to the directory object */
  ...
  for (;;) {
    ...
    w &= 0xFF;
    if (IsDBCS1(
            w)) { /* Check if it is a DBC 1st byte (always false on SBCS cfg) */
  ...
  *path = &p[si];               /* Return pointer to the next segment */
  ...
  for (;;) {
    ...
    if (_DF1S &&
        w >= 0x100) { /* Double byte character (always false on SBCS cfg) */
      if (i >= ni - 1) {
        cf |= NS_LOSS | NS_LFN;
        i = ni;
        continue;
  ...
  if (dp->fn[0] == DDE)
    dp->fn[0] = NDDE; /* If the first character collides with deleted mark,
  ...
  for (;;) {
    ...
    if (IsDBCS1(
            c)) { /* Check if it is a DBC 1st byte (always false on SBCS cfg) */
  ...
  *path = &p[si];               /* Return pointer to the next segment */
  ...
  if (sfn[0] == DDE)
    sfn[0] =
        NDDE; /* When first character collides with DDE, replace it with 0x05 */
  ...
  if (*path) { /* If the pointer is not a null */
    ...
    if (*tt == ':') { /* If a ':' is exist in the path name */
      ...
      if (i < 10 && tp == tt) { /* Is there a numeric drive id? */
        if (i <
            _VOLUMES) { /* If a drive id is found, get the value and strip it */
          vol = (int)i;
          *path = ++tt;
        }
...
static BYTE check_fs(/* 0:FAT boor sector, 1:Valid boor sector but not FAT,
  ...
  if (LD_WORD(&fs->win[BS_55AA]) !=
      0xAA55) /* Check boot record signature (always placed at offset 510 even
...
find_volume(/* FR_OK(0): successful, !=0: any error occurred */
  ...
  *rfs = fs;    /* Return pointer to the file system object */
  ...
  stat = eDisk_Init(fs->drv); /* Initialize the physical drive */
  ...
  if (fmt == 1 ||
      (!fmt &&
       (LD2PT(vol)))) { /* Not an FAT boot sector or forced partition number */
    UINT i;
    DWORD br[4];
...
validate(          /* FR_OK(0): The object is valid, !=0: Invalid */
  ...
  if (eDisk_Status(fil->fs->drv) & STA_NOINIT) return FR_NOT_READY;
  ...
  for (; btr; /* Repeat until all data read */
    ...
    if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
      ...
      if (cc) {              /* Read maximum contiguous sectors directly */
        ...
        if (eDisk_Read(fp->fs->drv, rbuff, sect, cc) != RES_OK)
...
#endif
        if (eDisk_Read(fp->fs->drv, fp->buf, sect, 1) !=
  ...
  for (; btw; /* Repeat until all data written */
    ...
    if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
      ...
      if (cc) {              /* Write maximum contiguous sectors directly */
        if (csect + cc > fp->fs->csize) /* Clip at cluster boundary */
          cc = fp->fs->csize - csect;
        if (eDisk_Write(fp->fs->drv, wbuff, sect, cc) != RES_OK)
          ABORT(fp->fs, FR_DISK_ERR);
#if _FS_MINIMIZE <= 2
#if _FS_TINY
...
FRESULT f_getcwd(TCHAR* buff, /* Pointer to the directory path */
  ...
  *buff = 0;
  ...
  if (res == FR_OK) {
    ...
    dj.sclust =
        dj.fs
            ->cdir; /* Start to follow upper directory from current directory */
    while ((ccl = dj.sclust) !=
...
#endif
          if (eDisk_Read(fp->fs->drv, fp->buf, dsc, 1) !=
  ...
  /* Normal Seek */
  {
    ...
    if (ofs) {
      ...
      if (ifptr > 0 &&
        ...
        fp->fptr =
            (ifptr - 1) & ~(bcs - 1); /* start from the current cluster */
        ofs -= fp->fptr;
        clst = fp->clust;
      ...
      if (clst != 0) {
        while (ofs > bcs) { /* Cluster following loop */
          ...
          if (fp->flag & FA_WRITE) { /* Check if in write mode or not */
            clst =
                create_chain(fp->fs, clst); /* Force stretch if in write mode */
...
#endif
            clst = get_fat(
                fp->fs, clst); /* Follow cluster chain if not in write mode */
          if (clst == 0xFFFFFFFF) ABORT(fp->fs, FR_DISK_ERR);
...
#endif
      if (eDisk_Read(fp->fs->drv, fp->buf, nsect, 1) !=
          RES_OK) /* Fill sector cache */
        ABORT(fp->fs, FR_DISK_ERR);
#endif
      fp->dsect = nsect;
      ...
      if (res == FR_OK) {
        ...
        if (res == FR_OK) {
          if (dp->sclust) {
            ...
            if (!dp->lockid) res = FR_TOO_MANY_OPEN_FILES;
          } else {
    ...
    if (res == FR_OK) { /* Old object is found */
      if (!djo.dir) {   /* Is root dir? */
        ...
        mem_cpy(buf, djo.dir + DIR_Attr,
                21); /* Save the object information except name */
        ...
        if (res == FR_NO_FILE) { /* It is a valid path and no name collision */
          /* Start of critical section that any interruption can cause a
           * cross-link */
          ...
          if (res == FR_OK) {
            ...
            if ((dir[DIR_Attr] & AM_DIR) &&
                djo.sclust != djn.sclust) { /* Update .. entry in the directory
                                               if needed */
...
FRESULT f_setlabel(const TCHAR* label /* Pointer to the volume label to set */
  ...
  if (sl) { /* Create volume label in directory form */
    ...
    do {
      ...
      w = (BYTE)label[i++];
      if (IsDBCS1(w))
        w = (j < 10 && i < sl && IsDBCS2(label[i])) ? w << 8 | (BYTE)label[i++]
                                                    : 0;
#if _USE_LFN
...
FRESULT f_mkfs(const TCHAR* path, /* Logical drive number */
  ...
  if (_MULTI_PARTITION && part) {
    ...
    if (eDisk_Read(pdrv, fs->win, 0, 1) != RES_OK) return FR_DISK_ERR;
  ...
  if (_MULTI_PARTITION && part) {
    ...
    tbl = &fs->win[MBR_Table + (part - 1) * SZ_PTE];
    tbl[4] = sys;
    if (eDisk_Write(pdrv, fs->win, 0, 1) != RES_OK) /* Write it to teh MBR */
      return FR_DISK_ERR;
    md = 0xF8;
  } else {
    if (sfd) { /* No partition table (SFD) */
      ...
      mem_set(fs->win, 0, SS(fs));
      tbl = fs->win + MBR_Table; /* Create partition table for single partition
                                    in the drive */
      ...
      if (eDisk_Write(pdrv, fs->win, 0, 1) != RES_OK) /* Write it to the MBR */
        return FR_DISK_ERR;
      md = 0xF8;
    }
  }
  ...
  if (eDisk_Write(pdrv, tbl, b_vol, 1) !=
      RES_OK) /* Write it to the VBR sector */
    return FR_DISK_ERR;
  if (fmt == FS_FAT32) /* Write backup VBR if needed (VBR+6) */
    eDisk_Write(pdrv, tbl, b_vol + 6, 1);
  ...
  for (i = 0; i < N_FATS; i++) { /* Initialize each FAT copy */
    ...
    if (eDisk_Write(pdrv, tbl, wsect++, 1) != RES_OK) return FR_DISK_ERR;
    mem_set(tbl, 0, SS(fs));      /* Fill following FAT entries with zero */
    for (n = 1; n < n_fat; n++) { /* This loop may take a time on FAT32 volume
                                     due to many single sector writes */
      if (eDisk_Write(pdrv, tbl, wsect++, 1) != RES_OK) return FR_DISK_ERR;
    }
  }
  ...
  if (fmt == FS_FAT32) {
    ...
    eDisk_Write(pdrv, tbl, b_vol + 1, 1); /* Write original (VBR+1) */
    eDisk_Write(pdrv, tbl, b_vol + 7, 1); /* Write backup (VBR+7) */
  }
...
FRESULT f_fdisk(
  ...
  /* Write it to the MBR */
  return (eDisk_Write(pdrv, buf, 0, 1) != RES_OK ||
