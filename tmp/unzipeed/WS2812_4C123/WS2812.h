...
#define USEDMA \
  1  // 0 for SysTick timing; non-zero for DMA Channel 8 and Timer 5A
//#define NUMCOLS 36  // number of LEDs per string (NOTE: See line 327 of
// WS2812.c in WS2812_PrintBuffer() where this is also hard-coded) #define
// NUMCOLS 96  // number of LEDs per string
#define NUMCOLS 240  // number of LEDs per string
#define NUMROWS \
  3  // number of LED strings (1<=NUMROWS<=8); strings connected to the
     // least-significant bits of Port E
#define ROWMASK     \
  ((1 << NUMROWS) - \
   1)  // bit mask for all rows for direction, alternate function, etc.
#define ROWMASK2                 \
  ((0xFFFFFFFF >> (4 * NUMROWS)) \
   << (4 * NUMROWS))  // bit mask for all rows for port control

enum colororder {
  GRB,  // send green first, then red, last blue
  RGB   // send red first, then green, last blue
};
