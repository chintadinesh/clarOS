...
#else
                              // Keil uVision Code
__asm void Delay(uint32_t ulCount) { subs r0, #1 bne Delay bx lr }
...
enum colororder CurrentColorOrder =
    GRB;  // seems to be the most common (see above), try this first
// enum colororder CurrentColorOrder = RGB; // the Flexible Silicone Neopixel
// LED Strip (Adafruit #4310) purchased fall 2020 has this encoding order
...
#if USEDMA
  DMA_Init(32, (uint32_t *)(0x40024000 +
                            (ROWMASK
                             << 2)));  // initialize DMA channel 8 for Timer5A
                                       // transfer, every 24 cycles (0.4 usec)
#else
...
void WS2812_AddColor(uint8_t red, uint8_t green, uint8_t blue, uint8_t row) {
  ...
  if (CurrentColorOrder == GRB) {
  ...
  } else if (CurrentColorOrder == RGB) {
    for (i = 0x80; i > 0x00; i = i >> 1) {  // store the red byte first
      if (red & i) {
        //        buffer[cursor[row]] |= mask;
        buffer[cursor[row] + 1] |= mask;
        //        buffer[cursor[row]+2] &= ~mask;
    ...
    for (i = 0x80; i > 0x00; i = i >> 1) {  // store the green byte second
      if (green & i) {
        //        buffer[cursor[row]] |= mask;
        buffer[cursor[row] + 1] |= mask;
        //        buffer[cursor[row]+2] &= ~mask;
...
void WS2812_PrintBuffer(void) {
  if (DMA_Status() !=
      IDLE) {  // handle error case where DMA channel is busy
  ...
  //  DMA_Transfer((uint8_t *)buffer, 1728, 2); // send 18 LEDs with each
  //  ping-pong; send 2 ping-pongs DMA_Transfer((uint8_t *)buffer, 1152, 8); //
  //  send 12 LEDs with each ping-pong; send 8 ping-pongs
  DMA_Transfer((uint8_t *)buffer, 1920,
               12);  // send 20 LEDs with each ping-pong; send 12 ping-pongs
}
#else
                   /*void WS2812_PrintBuffer(void){
                     uint32_t i;
                     ...
                     *portE = buffer[0];              // send the first byte
                     ...
                     for(i=1; i<(3*8*4*NUMCOLS); i=i+1){
                                                      // wait any remaining time from previous
                   delay                    while(((startTime-NVIC_ST_CURRENT_R)&0x00FFFFFF) <
                   24){};                    startTime =                    NVIC_ST_CURRENT_R;
                   *portE = buffer[i];
                     }
                                                      // wait any remaining time from previous
                   delay                    while(((startTime-NVIC_ST_CURRENT_R)&0x00FFFFFF) <
                   24){};                    EnableInterrupts();              // end of critical
                   section
                   }*/
...
#else
// Keil uVision Code
__asm void WS2812_PrintBuffer(void) {
  IMPORT buffer CPSID I PUSH{R4, R5, R6, R7, R8, R9, R10, R11}
  // save current value of R4-R11
  LDR R4,
      = 0xE000E010  // R4 = &NVIC_ST_CTRL_R (pointer)
      LDR R5,
      = 0x00010005  // R5 = SysTick control value
      LDR R6,
      = 0xE000E014  // R6 = &NVIC_ST_RELOAD_R (pointer)
      MOV R7,
# 16      // R7 = SysTick reload value
      LDR R8,
      = 0xE000E018  // R8 = &NVIC_ST_CURRENT_R (pointer)
      LDR R9,
      = buffer  // R9 = &buffer (pointer)
          LDR R10,
      = 0x40024000  // R10 = &GPIO_PORTE_BASE (pointer)
      MOV R0,
#ROWMASK  // R0 = ROWMASK (value)
      LSL R0, R0,
# 2       // R0 = R0<<2
      ADD R10, R10,
      R0  // R10 = R10 + R0 (&GPIO_PORTE_BASE + ROWMASK<<2) (pointer)
          MOV R11,
# 0       // R11 = 0 (number of runs through loop)
      WS2812_PrintBufferMain LDRB R0, [R9],
# 1 // R0 = buffer (8-bit value); R9 = R9 + 1 (increment buffer pointer to \
    // next byte)
      ADD R11, R11,
# 1 // R11 = R11 + 1 (increment counts through loop)
      STR R7,
      [R6]  // [R6] = R7 (NVIC_ST_RELOAD_R = 16)
      STR R7,
      [R8]  // [R8] = R7 (NVIC_ST_CURRENT_R = 16; any write clears it)
      STR R0,
      [R10]  // [R10] = R0 (GPIO_PORTE = buffer[R11])
      WS2812_PrintBufferWait LDR R0,
      [R4]  // R0 = NVIC_ST_CTRL_R (value)
      CMP R0,
      R5  // is R0 (NVIC_ST_CTRL_R) == R5 (0x00010005) ?
          BNE WS2812_PrintBufferWait  // if not, branch to
                                      // 'WS2812_PrintBufferWait'
              CMP R11,
#(3 * 8 * 4 * NUMCOLS) // is R11 (runs through loop) < (3*8*4*NUMCOLS) \
                       // (maximum runs through loop) ?
      BLO WS2812_PrintBufferMain  // if so, branch to 'WS2812_PrintBufferMain'
          POP { R4, R5, R6, R7, R8, R9, R10, R11 }
  // restore previous value of R4-R11
  CPSIE I BX LR  // return
}
