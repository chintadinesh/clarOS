...
// WS2812 data connected to PE0 through 300 to 500 ohm resistor and level
// shifter circuit 1,000 uF, 6.3 V capacitor across power rails
...
//    *WS2812 LED string GND, LaunchPad GND, level shifter GND, black banana
//    plug to power supply -, middle pin of 78S05 regulator *LaunchPad +3.3 V,
//    level shifter LV *LaunchPad PE0, level shifter LV1 *level shifter LV1 to
//    one side of 330 ohm resistor *other side of 330 ohm resistor to WS2812 LED
//    string DIN
//    *>=1,000 uF capacitor between right and middle pin of 78S05 regulator
//    (check polarity) *4.7 uF capacitor between left and middle pin of 78S05
//    regulator (check polarity)
// 2a) Before applying power, connect the following together to use the
// regulator:
...
// 2b) Before applying power, connect the following together to bypass the
// regulator:
//    *power supply + red banana plug to +5 V from step 1)
//    *disconnect red banana plug from power supply + for now
// 3a) Turn on power supply, turn on output, set voltage to 7.4 V, connect red
// banana plug to power supply + 3b) Turn on power supply, turn on output, set
// voltage to 5.0 V, connect red banana plug to power supply +
...
// Input: scale  LED outputs are right shifted by this amount (0<=scale<=7) to
...
void Pattern_SolidRainbow(uint8_t scale, uint8_t row);
void Pattern_Bulbs(uint8_t scale, uint8_t row);
void Pattern_Stacker(uint8_t red, uint8_t green, uint8_t blue, uint32_t delay,
                     uint8_t row);
void Pattern_ThreeRowSquare(uint8_t waveRed, uint8_t waveGreen,
                            uint8_t waveBlue, uint8_t bgRed, uint8_t bgGreen,
                            uint8_t bgBlue);
void Pattern_ThreeRowTriangle(uint8_t waveRed, uint8_t waveGreen,
                              uint8_t waveBlue, uint8_t bgRed, uint8_t bgGreen,
                              uint8_t bgBlue);
void Pattern_ThreeRowSine(uint8_t waveRed, uint8_t waveGreen, uint8_t waveBlue,
...
void Pattern_SplitMeterStriped(uint8_t litLEDs, uint8_t LEDsPerSide,
                               uint8_t row);
void Pattern_SplitMeterColored(uint8_t litLEDs, uint8_t LEDsPerSide,
                               uint8_t row);
...
// Output: none
void Random_Init(uint32_t seed) { M = seed; }
...
int main_hat(void) {
  ...
  WS2812_Init();  // set system clock to 80 MHz, initialize SysTick, initialize
                  // PE0 and make it output
  // **** ADC0 initialization ****
  ...
  TIMER1_TAMR_R =
      0x0002;  // configure for periodic mode, default down-count settings
  ...
  NVIC_EN0_R =
      1 << 21;  // enable interrupt 21 in NVIC
                // Timer5A interrupt on DMA completion;
                // ADC0 Sequencer 3 interrupt on microphone sample complete;
  EnableInterrupts();  // Timer1A interrupt at 50 Hz accelerometer sampling
                       // frequency
  WS2812_ClearBuffer();
  WS2812_SendReset();
  ...
  while (1) {
    ...
    //    Pattern_Reset();
    //    WS2812_ClearBuffer();
    //    while((SWITCHES == (SW1|SW2) && (run < maxrun))){ // repeat while
    //    switches released or for about 20 seconds
    ...
    //    maxrun = 3;
    //    while((SWITCHES == (SW1|SW2)) && (run < maxrun)){ // repeat while
    //    switches released or until sequence completes 3 times
    //      Pattern_Reset();
    //      WS2812_ClearBuffer();
    //      i = 0;
    ...
    //      while((i < NUMCOLS) && (SWITCHES == (SW1|SW2))){
    //        i = i + 1;
    //        Pattern_Stacker(Random()>>24, Random()>>24, Random()>>24, 266666,
    //        0);
    //      }
    //      if(i >= NUMCOLS){            // skip the delay if the above loops
    //      broke due to button pressed
    //        Delay(133333333);          // delay about 5 sec to observe random
    //        strand
    //      }
    //      run = run + 1;
    //    }
    ...
    while (
        (SWITCHES == (SW1 | SW2)) &&
        (run <
         maxrun)) {  // repeat while switches released or for about 20 seconds
    ...
    while (
        (SWITCHES == (SW1 | SW2)) &&
        (run <
         maxrun)) {  // repeat while switches released or for about 18 seconds
      i = 0;
...
// NOTE: Be sure to change line 327 in WS2812.c and line 32 in
// WS2812.h to accomodate 240 LEDs per strand.
uint32_t PatternDelay = 0;    // decrements 100 times per second
void Timer1A_Handler(void) {  // void Timer1A_Handler_ChristmasTree(void){
  static int counter = 24;    // local variable counts number of interrupts
  TIMER1_ICR_R = TIMER_ICR_TATOCINT;  // acknowledge timer0A timeout
  if (PatternDelay > 0) {
    PatternDelay = PatternDelay - 1;
  }
  counter = counter - 1;
  if (counter == 0) {
    counter = 24;
    LEDS ^= GREEN;  // toggle green LED (PF3) at about 2 Hz
  }
}

int main(void) {  // int main_ChristmasTree(void){
  uint32_t run, maxrun;
  // **** LED string initialization ****
  WS2812_Init();  // set system clock to 80 MHz, initialize SysTick, initialize
                  // PE2-0 and make them output
  // **** GPIO Port F initialization ****
  // activate clock for Port F
  SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R5;
  // allow time for clock to stabilize
  ...
  TIMER1_TAMR_R =
      0x0002;  // configure for periodic mode, default down-count settings
  TIMER1_TAPR_R = 24;      // prescale value for 100 Hz interrupt
  TIMER1_TAILR_R = 31999;  // reload value for 100 Hz interrupt
  TIMER1_ICR_R = 0x0001;   // clear TIMER1A timeout flag
  TIMER1_IMR_R |= 0x0001;  // arm timeout interrupt
  TIMER1_CTL_R |= 0x0001;  // enable Timer1A 16b, periodic, interrupts
  // **** NVIC interrupt initialization ****
  ...
  NVIC_EN0_R = 1 << 21;  // enable interrupt 21 in NVIC
                         // Timer5A interrupt on DMA completion;
  EnableInterrupts();    // Timer1A interrupt at 100 Hz delay counter
  WS2812_ClearBuffer();
  WS2812_SendReset();
  WS2812_AddColor(0xAA, 0xAA, 0xAA, 0);
  WS2812_AddColor(64, 0, 0, 0);
  ...
  WS2812_AddColor(128, 0, 0, 2);  // third LED of row 2 red
  WS2812_PrintBuffer();
  // if the LaunchPad is packaged in a box, skip this section
  /*  // wait for switch pressed
    while(SWITCHES == (SW1|SW2)){
      WaitForInterrupt();
    }
  ...
  while (1) {
    ...
    while (
        (SWITCHES == (SW1 | SW2) &&
         (run <
          maxrun))) {  // repeat while switches released or for about 4 minutes
      Pattern_RainbowWaves(1, NUMCOLS, 0);
      // decrement 'phase' since it should only be incremented once per three
      // calls
      if (phase == 0) {
        phase = NUMCOLS - 1;
      } else {
      ...
      Pattern_RainbowWaves(1, NUMCOLS, 1);
      // decrement 'phase' since it should only be incremented once per three
      // calls
      if (phase == 0) {
        phase = NUMCOLS - 1;
      ...
      Pattern_RainbowWaves(1, NUMCOLS, 2);
    ...
    while ((SWITCHES == (SW1 | SW2)) &&
           (run <
            maxrun)) {  // repeat while switches released or for about 2 minutes
      Pattern_WhiteWaves(2, 0);
      // decrement 'phase' since it should only be incremented once per three
      // calls
      if (phase == 0) {
...
// Input: none
// Output: none
void Pattern_Reset(void) { phase = 0; }
...
void Pattern_RainbowWaves(uint8_t scale, uint8_t cols, uint8_t row) {
  ...
  increment = 255 / (cols / 3);  // constant amount of color added or subtracted
                                 // from one LED to the next
  maximum =
      increment * cols / 3;  // largest amount of color any LED will reach (i.e.
                             // (maximum, 0, 0) is the "reddest" LED)
  if (phase < (cols / 3)) {
...
//------------Pattern_SolidRainbow------------
// Color each LED in the strand a single solid color.
// Subsequent calls step this color through a rainbow.  Each call
// draws the pattern once by updating the RAM buffer and sending
// it once.  An additional external delay is advised.
// Input: scale  LED outputs are right shifted by this amount (0<=scale<=7) to
// reduce current/brightness
...
void Pattern_SolidRainbow(uint8_t scale, uint8_t row) {
  ...
  WS2812_SetCursor(0, row);
  increment = 32;  // constant amount of color added or subtracted from one LED
                   // to the next
  maximum = 224;   // largest amount of color any LED will reach (i.e. (maximum,
                   // 0, 0) is the "reddest" LED)
  if (phase < 8) {
    red = maximum - increment * phase;
    green = increment * phase;
    blue = 0;
    ...
    red = 0;
    green = maximum - increment * (phase - 8);
    blue = increment * (phase - 8);
  } else {
    red = increment * (phase - 16);
    green = 0;
    blue = maximum - increment * (phase - 16);
  }
  phase = phase + 1;
  if (phase > 23) {
    phase = 0;
...
//------------Pattern_Bulbs------------
// Mimic a set of bulbs by coloring a few LEDs and then skipping
// a few.  Subsequent calls alternate colored and skipped
// positions.  Each call draws the pattern once by updating the
// RAM buffer and sending it once.  An additional external delay
// is advised.
// Input: scale  LED outputs are right shifted by this amount (0<=scale<=7) to
// reduce current/brightness
//        row    index of LED row (0<=row<NUMROWS)
// Output: none
/*#define BULBSSIZE 8
const uint8_t BULBS[BULBSSIZE][3] = {
  {0xFF, 0x00, 0x00},
  {0xFF, 0xFF, 0x00},
  {0x00, 0xFF, 0x00},
  {0x00, 0xFF, 0xFF},
  ...
  {0x00, 0x00, 0x00}};*/
#define BULBSSIZE 10
const uint8_t BULBS[BULBSSIZE][3] = {{0xFF, 0x00, 0x00}, {0x00, 0xFF, 0x00},
                                     {0x00, 0x00, 0xFF}, {0xFF, 0x00, 0x00},
                                     {0x00, 0xFF, 0x00}, {0x00, 0x00, 0xFF},
                                     {0xFF, 0x00, 0x00}, {0x00, 0xFF, 0x00},
                                     {0x00, 0x00, 0xFF}, {0xFF, 0xFF, 0x00}};
void Pattern_Bulbs(uint8_t scale, uint8_t row) {
  uint32_t i, index, endcol;
  if (row >= NUMROWS) {
    return;  // row parameter is invalid
  }
  WS2812_SetCursor(0, row);
  phase = phase + 1;
  index = phase & 0x01;
  endcol = 8 * (NUMCOLS / 8);  // round if this is not a multiple of 8
  for (i = 0; i < endcol; i = i + 8) {
    if ((phase & 0x01) == 0) {
      // 'bulb' then 'blank', index: 0, 2, 4, ...
      WS2812_AddColor(BULBS[index][0] >> (scale + 2),
                      BULBS[index][1] >> (scale + 2),
                      BULBS[index][2] >> (scale + 2), row);
      WS2812_AddColor(BULBS[index][0] >> scale, BULBS[index][1] >> scale,
                      BULBS[index][2] >> scale, row);
      WS2812_AddColor(BULBS[index][0] >> scale, BULBS[index][1] >> scale,
                      BULBS[index][2] >> scale, row);
      WS2812_AddColor(BULBS[index][0] >> (scale + 2),
                      BULBS[index][1] >> (scale + 2),
...
void Pattern_ThreeRowTriangle(uint8_t waveRed, uint8_t waveGreen,
  ...
  // The total amount of color in all LEDs is:
  total =
      (waveRed + waveGreen + waveBlue + 2 * bgRed + 2 * bgGreen + 2 * bgBlue) *
      NUMCOLS;
  ...
  for (i = 1; i <= NUMCOLS; i = i + 1) {
    if (litLEDs == 0) {  // nothing to display; no LEDs on
      WS2812_AddColor(0, 0, 0, row);
               ...
               (litLEDs - 1)) {  // LEDs near beginning of strand slightly on
      WS2812_AddColor(PWR_BASIC_RED >> 3, PWR_BASIC_GRN >> 3,
                      PWR_BASIC_BLU >> 3, row);
    } else if (i == (litLEDs - 1)) {  // LED near end of bar slightly brighter
      WS2812_AddColor(PWR_BASIC_RED >> 1, PWR_BASIC_GRN >> 1,
                      PWR_BASIC_BLU >> 1, row);
  ...
  for (i = 1; i <= NUMCOLS; i = i + 1) {
    if (litLEDs == 0) {  // nothing to display; no LEDs on
      WS2812_AddColor(0, 0, 0, row);
               ...
               (litLEDs - 1)) {  // LEDs near beginning of strand slightly on
      if (i <= PWR_STRIPE0) {
        WS2812_AddColor(PWR_STRIPE0_RED >> 3, PWR_STRIPE0_GRN >> 3,
                        PWR_STRIPE0_BLU >> 3, row);
      } else if (i <= (PWR_STRIPE0 + PWR_STRIPE1)) {
        WS2812_AddColor(PWR_STRIPE1_RED >> 3, PWR_STRIPE1_GRN >> 3,
                        PWR_STRIPE1_BLU >> 3, row);
      } else if (i <= (PWR_STRIPE0 + PWR_STRIPE1 + PWR_STRIPE2)) {
        WS2812_AddColor(PWR_STRIPE2_RED >> 3, PWR_STRIPE2_GRN >> 3,
                        PWR_STRIPE2_BLU >> 3, row);
      } else {
        WS2812_AddColor(PWR_STRIPE3_RED >> 3, PWR_STRIPE3_GRN >> 3,
                        PWR_STRIPE3_BLU >> 3, row);
      }
    } else if (i == (litLEDs - 1)) {  // LED near end of bar slightly brighter
      if (i <= PWR_STRIPE0) {
        WS2812_AddColor(PWR_STRIPE0_RED >> 1, PWR_STRIPE0_GRN >> 1,
                        PWR_STRIPE0_BLU >> 1, row);
      } else if (i <= (PWR_STRIPE0 + PWR_STRIPE1)) {
        WS2812_AddColor(PWR_STRIPE1_RED >> 1, PWR_STRIPE1_GRN >> 1,
                        PWR_STRIPE1_BLU >> 1, row);
      } else if (i <= (PWR_STRIPE0 + PWR_STRIPE1 + PWR_STRIPE2)) {
        WS2812_AddColor(PWR_STRIPE2_RED >> 1, PWR_STRIPE2_GRN >> 1,
                        PWR_STRIPE2_BLU >> 1, row);
      } else {
        WS2812_AddColor(PWR_STRIPE3_RED >> 1, PWR_STRIPE3_GRN >> 1,
                        PWR_STRIPE3_BLU >> 1, row);
      }
    } else if (i == litLEDs) {  // LED at end of bar full brightness
      if (i <= PWR_STRIPE0) {
...
// Input: litLEDs     number of LEDs to illuminate
// (litLEDs<=LEDsPerSide<=(NUMCOLS/2))
//        LEDsPerSide number of LEDs on each side of the meter
//        row         index of LED row (0<=row<NUMROWS)
// Output: none
void Pattern_SplitMeterBasic(uint8_t litLEDs, uint8_t LEDsPerSide,
                             uint8_t row) {
  uint32_t i;
  if (row >= NUMROWS) {
    return;  // row parameter is invalid
  ...
  if (LEDsPerSide >
      (NUMCOLS / 2)) {          // check that the whole meter fits in the LEDs
    ...
    if (litLEDs == 0) {                       // nothing to display; no LEDs on
      ...
      if (i <= (PWR_STRIPE0 / 2)) {
        ...
        WS2812_AddColor(PWR_STRIPE1_RED >> 3, PWR_STRIPE1_GRN >> 3,
                 ...
                 ((PWR_STRIPE0 / 2) + (PWR_STRIPE1 / 2) + (PWR_STRIPE2 / 2))) {
        WS2812_AddColor(PWR_STRIPE2_RED >> 3, PWR_STRIPE2_GRN >> 3,
                        PWR_STRIPE2_BLU >> 3, row);
      } else {
        WS2812_AddColor(PWR_STRIPE3_RED >> 3, PWR_STRIPE3_GRN >> 3,
