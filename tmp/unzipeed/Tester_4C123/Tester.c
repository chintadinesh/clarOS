...
#define SW1PRESS \
  0x01  // on the left side of the LaunchPad board (value of SWITCHES when only
        // SW1 pressed)
#define SW2PRESS \
  0x10  // on the right side of the LaunchPad board (value of SWITCHES when only
        // SW2 pressed)
...
unsigned short printLines =
    0;  // number of lines of error messages printed to UART
#define MAXLINES \
  20  // maximum number of lines of error messages printed to UART
...
// to the UART.
// Input: firstPinMask    bit mask of the first pin to be tested (ex. 0x10 for
// PC)
//        lastPinMask     bit mask of the last pin to be tested (ex. 0x80 for
//        PC) allPinMask      bit mask of all pins to be tested, including the
//        first and last pins (ex. 0xF0 for PC) firstPinNumber  bit number of
//        first pin to be tested (ex. 4 for PC) portLetter      letter of port
//        to be tested (ex. 'C' for PC) dataRegPtr      pointer to the data
//        register of the port to be tested (ex. &GPIO_PORTC for PC)
//        directionRegPtr pointer to the direction register of the port to be
//        tested (ex. &GPIO_PORTC_DIR_R for PC)
// Output: char containing bit mask of possibly broken pins (ex. 0x30 if PC5-4
// is broken)
char digitaltest(unsigned long firstPinMask, unsigned long lastPinMask,
                 unsigned long allPinMask, unsigned long firstPinNumber,
                 unsigned char portLetter, volatile unsigned long *dataRegPtr,
  ...
  while (outmask <= lastPinMask) {
    ...
    *dataRegPtr = outmask;  // Pn[outmask] = 1
    ...
    if ((*dataRegPtr & allPinMask) != allPinMask) {
      ...
      while (inmask <= lastPinMask) {
        ...
        if (((*dataRegPtr & inmask) == 0) && (inmask != outmask) &&
            (printLines < MAXLINES)) {
    ...
    *dataRegPtr = 0;  // Pn = 0
    ...
    if ((*dataRegPtr & allPinMask) != 0x00) {
      ...
      while (inmask <= lastPinMask) {
        // expect each input to also be high
        if (((*dataRegPtr & inmask) == inmask) && (inmask != outmask) &&
            (printLines < MAXLINES)) {
          UART_OutString("GPIO output ");
          UART_OutChar(portLetter);
          UART_OutUDec(obit);
...
int main(void) {
  ...
  unsigned char digitalA = 0, digitalB = 0, digitalC = 0, digitalD = 0,
                digitalE = 0, digitalF = 0;
  ...
  GPIO_PORTA_AMSEL_R =
      0;  // disable analog functionality on PA (no analog on PA)
  ...
  GPIO_PORTB_AMSEL_R &=
      ~0x30;  // disable analog functionality on PB5-4 (AIN10 and AIN11)
  ...
  GPIO_PORTC_AMSEL_R &=
      ~0xF0;  // disable analog functionality on PC7-4 (C0-, C0+, C1+, and C1-)
  ...
  GPIO_PORTD_AMSEL_R &= ~0x0F;  // disable analog functionality on PD3-0 (AIN4,
                                // AIN5, AIN6, and AIN7)
  ...
  GPIO_PORTE_AMSEL_R &= ~0x3F;  // disable analog functionality on PE5-0 (AIN8,
                                // AIN9, AIN0, AIN1, AIN2, and AIN3)
  ...
  GPIO_PORTF_AMSEL_R =
      0;  // disable analog functionality on PF (no analog on PF)
  // 9) activate PLL for 50 MHz clock
  PLL_Init();
  // 10) activate UART0 at 115,200 baud rate, 8 data bits, no parity, 1 stop
  // bit, no hardware flow control
  UART_Init();
  UART_OutString("\r\nLM4F120/TM4C123 Test\r\n");
  // 11) test Port A digital
  digitalA =
      digitaltest(0x04, 0x80, 0xFC, 2, 'A', &GPIO_PORTA, &GPIO_PORTA_DIR_R);
  ...
  digitalB =
      digitaltest(0x01, 0x80, 0xFF, 0, 'B', &GPIO_PORTB, &GPIO_PORTB_DIR_R);
  // 13) test Port C digital
  digitalC =
      digitaltest(0x10, 0x80, 0xF0, 4, 'C', &GPIO_PORTC, &GPIO_PORTC_DIR_R);
  // 14) test Port D digital
  digitalD |=
      digitaltest(0x01, 0x08, 0x0F, 0, 'D', &GPIO_PORTD, &GPIO_PORTD_DIR_R);
  digitalD |=
      digitaltest(0x40, 0x80, 0xC0, 6, 'D', &GPIO_PORTD, &GPIO_PORTD_DIR_R);
  // 15) test Port E digital
  digitalE =
      digitaltest(0x01, 0x20, 0x3F, 0, 'E', &GPIO_PORTE, &GPIO_PORTE_DIR_R);
  // 16) test Port F digital
  digitalF =
      digitaltest(0x01, 0x10, 0x1F, 0, 'F', &GPIO_PORTF, &GPIO_PORTF_DIR_R);
  ...
  while (channel <= 11) {
    ...
    previous = ADC0_InSeq3();  // take ADC sample
                               // ***Important: This should measure
                               // approximately zero. If a digital output pin is
                               // broken high (always +3.3), this will be
                               // non-zero and may cause a false-positive error.
    ...
    while (outmask <= 0x80) {
      ...
      } while (((digitalB & outmask) || (outmask == inmask)) &&
               (outmask <= 0x80));
    }
  ...
  GPIO_PORTB_DEN_R |= 0xFF;     // enable digital I/O on PB7-0
  GPIO_PORTB_AMSEL_R &=
      ~0x30;  // disable analog functionality on PB5-4 (AIN11 and AIN10)
  ...
  while (channel >= 4) {
    ...
    while (outmask <= 0x80) {
      ...
      } while (((digitalD & outmask) || (outmask == inmask)) &&
               (outmask <= 0x80));
    }
    inmask = inmask << 1;   // check next analog input
    channel = channel - 1;  // check next analog input
  ...
  GPIO_PORTD_AMSEL_R &= ~0x0F;  // disable analog functionality on PD3-0 (AIN4,
                                // AIN5, AIN6, and AIN7)
  // 19) test Port E analog (PE5=AIN8, PE4=AIN9, PE3=AIN0, PE2=AIN1, PE1=AIN2,
  // and PE0=AIN3)
  inmask = 0x01;  // bit mask of analog input being tested
  ...
  while (inmask <= 0x20) {
    ...
    // disable analog functionality on PE5-0 (AIN8, AIN9, AIN0, AIN1, AIN2, and
    // AIN3)
    ...
    while (outmask <= 0x20) {
      ...
      do {
        ...
        // if this pin is the analog input, go to the next one
        outmask = outmask << 1;  // if run out of pins, break out of loop
      } while (((digitalE & outmask) || (outmask == inmask)) &&
               (outmask <= 0x20));
    }
    inmask = inmask << 1;  // check next analog input
  ...
  UART_OutString("\r\n");
  if ((digitalA == 0xFC) && (digitalB == 0xFF) && (digitalC == 0xF0) &&
      (digitalD == 0xCF) && (digitalE == 0x3F) && (digitalF == 0x1F)) {
  ...
  } else if ((digitalA > 0) || (digitalB > 0) || (digitalC > 0) ||
             (digitalD > 0) || (digitalE > 0) || (digitalF > 0)) {
