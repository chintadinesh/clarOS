...
uint32_t Port_In(void) { return 0; }
void FSM(void) {
  StatePtr Pt;
  uint8_t in;
  Pt = SA;  // Initial State
...
//******************PID**************************
void PID_Init(void) {}
uint8_t PID_In(void) { return 0; }
uint8_t PID_Calc(uint8_t speed) { return speed; }
...
//******************DAS**************************
void DAS_Init(void) {}
uint8_t DAS_In(void) { return 0; }
uint8_t DAS_Calc(uint8_t speed) { return speed; }
void DAS(void) {
  uint8_t raw, Result;
  DAS_Init();  // Initialize
  for (;;) {
...
TCBType sys[4] = {{&sys[0].InitialReg[0], {0}, {0}, (uint32_t)FSM, 0x01000000},
