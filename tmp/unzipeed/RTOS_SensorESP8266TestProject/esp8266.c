    ...
    case 4:  // filter out data and put it into the right receive FIFO
      if (ReceiveDataLen) {
        switch (ReceiveDataStream) {
          case 0:
            if (ESP8266Rx0Fifo_Put(letter) ==
                FIFOFAIL) {  // overflow, data loss
...
// Inputs: none
// Outputs: none
void ESP8266_EnableInterrupt(void) { UART_ESP8266(_EnableRXInterrupt)(); }

//--------ESP8266_DisableInterrupt--------
// Disables uart interrupt
// Inputs: none
// Outputs: none
void ESP8266_DisableInterrupt(void) { UART_ESP8266(_DisableRXInterrupt)(); }
...
void static ESP8266RxToBuffer(void) {
  char letter;
  while (((UART_ESP8266(_FR_R) & UART_FR_RXFE) == 0) &&
         (ESP8266RxFifo_Size() < (FIFOSIZE - 1))) {
...
void static ESP8266BufferToTx(void) {
  char letter;
  while (((UART_ESP8266(_FR_R) & UART_FR_TXFF) == 0) &&
         (ESP8266TxFifo_Size() > 0)) {
...
int ESP8266_Init(int rx_echo, int tx_echo) {
  ...
  while (*s) {
    while (timer && ((UART_ESP8266(_FR_R) & UART_FR_RXFE) != 0)) {
      timer--;
    }
    if (!timer) break;
    ...
    if (rx_echo)
      UART_OutCharNonBlock(
          c);  // echo, requires UART0 to be operational, will print garbage
    if (c == *s) {
      s++;
    } else {
      s = ESP8266_READY_RESPONSE;
...
// The ESP only seems to have limited SSL support, does not work with all
// servers Inputs: IP address or web page as a string, port, and keepalive time
// (0 if none) output: 1 if success, 0 if fail
int ESP8266_MakeTCPConnection(char* IPaddress, uint16_t port,
                              uint16_t keepalive, int ssl) {
  int try = MAXTRY;
  char TXBuffer[32];
  while (try) {
    if (ssl) {
...
int ESP8266_Send(const char* fetch) {
  ...
  if (ESP8266_WaitForResponse(ESP8266_SENDOK_RESPONSE, ESP8266_ERROR_RESPONSE))
    return SUCCESS;
  return FAILURE;
}
