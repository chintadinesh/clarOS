...
void ReadByteData(void) {  // called on a SNP Characteristic Read Indication for
                           // characteristic ByteData
  ByteData++;
  LaunchPad_Output(ByteData);  // toggle LED
  OutValue("\n\rRead ByteData=", ByteData);
}
void ReadHalfWordData(void) {  // called on a SNP Characteristic Read Indication
                               // for characteristic HalfWordData
  LaunchPad_Output(ByteData ^ GREEN);  // toggle green color
  HalfWordData = LaunchPad_Input();
  OutValue("\n\rRead HalfWord=", HalfWordData);
}
void WriteByteData(void) {  // called on a SNP Characteristic Write Indication
                            // on characteristic ByteData
  LaunchPad_Output(ByteData ^ BLUE);  // toggle blue color
  OutValue("\n\rWrite ByteData=", ByteData);
}
void WriteWordData(void) {  // called on a SNP Characteristic Write Indication
                            // on characteristic WordData
...
int main(void) {
  ...
  AP_AddCharacteristic(0xFFF1, 1, &ByteData, 0x03, 0x0A, "ByteData",
                       &ReadByteData, &WriteByteData);
  //------------------------
  HalfWordData = LaunchPad_Input();  // read only parameter (get from switches)
  AP_AddCharacteristic(0xFFF2, 2, &HalfWordData, 0x01, 0x02, "HalfWordData",
                       &ReadHalfWordData, 0);
  //------------------------
  WordData = 0;  // write only parameter (sent to LED)
  AP_AddCharacteristic(0xFFF3, 4, &WordData, 0x02, 0x08, "WordData", 0,
                       &WriteWordData);
  //------------------------
  Switch1 = 0;
