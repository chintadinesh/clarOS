 ...
 * ----------
 * If you find any bug or problem, please create new issue or a pull request
 * with a fix in the repository. Or you can simply email me about the problem or
 * bug at zeelivermorium@gmail.com Much Appreciated!
 * ----------
...
VL53L0X_Error status = VL53L0X_ERROR_NONE;  // indicates whether or not the
                                            // sensor has encountered an error
...
#define VL53L0X_I2C_INIT
  ...
  if (!(I2C_address == VL53L0X_I2C_ADDR))
    if (!VL53L0X_setAddress(I2C_address)) return FAIL;
  ...
  if (status == VL53L0X_ERROR_NONE) {
    ...
    status = VL53L0X_PerformRefSpadManagement(&VL53L0X_device, &refSpadCount,
                                              &isApertureSpads);
  }
  ...
  if (status == VL53L0X_ERROR_NONE) {
    ...
    status =
        VL53L0X_PerformRefCalibration(&VL53L0X_device, &VhvSettings, &PhaseCal);
  }

  if (status == VL53L0X_ERROR_NONE) {
    // setup in single ranging mode
    status = VL53L0X_SetDeviceMode(&VL53L0X_device,
  ...
  if (status == VL53L0X_ERROR_NONE) {
    ...
    status = VL53L0X_SetLimitCheckEnable(
        &VL53L0X_device, VL53L0X_CHECKENABLE_SIGMA_FINAL_RANGE, 1);
  }

  if (status == VL53L0X_ERROR_NONE) {
    // enable signal check
    status = VL53L0X_SetLimitCheckEnable(
        &VL53L0X_device, VL53L0X_CHECKENABLE_SIGNAL_RATE_FINAL_RANGE, 1);
  }

  if (status == VL53L0X_ERROR_NONE) {
    // enable range check
    status = VL53L0X_SetLimitCheckEnable(
        &VL53L0X_device, VL53L0X_CHECKENABLE_RANGE_IGNORE_THRESHOLD, 1);
  }

  if (status == VL53L0X_ERROR_NONE) {
    // set limit check value
    status = VL53L0X_SetLimitCheckValue(
        &VL53L0X_device, VL53L0X_CHECKENABLE_RANGE_IGNORE_THRESHOLD,
...
int VL53L0X_setAddress(uint8_t newAddress) {
  ...
  status = VL53L0X_SetDeviceAddress(&VL53L0X_device,
                                    newAddress * 2);  // [7:2] is address
 ...
 */
VL53L0X_Error VL53L0X_getSingleRangingMeasurement(
    VL53L0X_RangingMeasurementData_t* RangingMeasurementData) {
  return VL53L0X_PerformSingleRangingMeasurement(&VL53L0X_device,
