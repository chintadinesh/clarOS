...
int VL53L0X_read_multi(uint8_t deviceAddress, uint8_t index, uint8_t *pdata,
  ...
  I2C0_MSA_R =
      (deviceAddress << 1) & I2C_MSA_SA_M;  // MSA[7:1] is slave address
  ...
  switch (count) {
    ...
    default:
      do {
        ...
        pdata[count - 1] =
            (I2C0_MDR_R & I2C_MDR_DATA_M);  // least significant byte
        retryCounter++;                     // increment retry counter
      }                                     // repeat if error
      while (((I2C0_MCS_R & (I2C_MCS_ADRACK | I2C_MCS_ERROR)) != 0) &&
             (retryCounter <= MAXRETRIES));
  ...
  switch (count) {
    ...
    default:
      do {
        ...
        pdata[count - 1] =
            (I2C1_MDR_R & I2C_MDR_DATA_M);  // least significant byte
        retryCounter++;                     // increment retry counter
      }                                     // repeat if error
      while (((I2C1_MCS_R & (I2C_MCS_ADRACK | I2C_MCS_ERROR)) != 0) &&
             (retryCounter <= MAXRETRIES));
  ...
  switch (count) {
    case 1:
      ...
      while (((I2C2_MCS_R & (I2C_MCS_ADRACK | I2C_MCS_ERROR)) != 0) &&
             (retryCounter <= MAXRETRIES));
    ...
    default:
      do {
        ...
        pdata[count - 1] =
            (I2C2_MDR_R & I2C_MDR_DATA_M);  // least significant byte
        retryCounter++;                     // increment retry counter
      }                                     // repeat if error
      while (((I2C2_MCS_R & (I2C_MCS_ADRACK | I2C_MCS_ERROR)) != 0) &&
             (retryCounter <= MAXRETRIES));
  ...
  switch (count) {
    case 1:
      ...
      while (((I2C3_MCS_R & (I2C_MCS_ADRACK | I2C_MCS_ERROR)) != 0) &&
             (retryCounter <= MAXRETRIES));
    ...
    default:
      do {
        ...
        pdata[count - 1] =
            (I2C3_MDR_R & I2C_MDR_DATA_M);  // least significant byte
        retryCounter++;                     // increment retry counter
      }                                     // repeat if error
      while (((I2C3_MCS_R & (I2C_MCS_ADRACK | I2C_MCS_ERROR)) != 0) &&
             (retryCounter <= MAXRETRIES));
...
int VL53L0X_write_multi(uint8_t deviceAddress, uint8_t index, uint8_t *pdata,
  ...
  if (count == 1) {
    ...
    for (int i = 0; i < count - 1; i++) {
      ...
      // check error bits
      if ((I2C2_MCS_R & (I2C_MCS_DATACK | I2C_MCS_ADRACK | I2C_MCS_ERROR)) !=
          0) {
        I2C2_MCS_R = I2C_MCS_STOP;  // stop transmission
        // return error bits if nonzero
        return (I2C2_MCS_R & (I2C_MCS_DATACK | I2C_MCS_ADRACK | I2C_MCS_ERROR));
