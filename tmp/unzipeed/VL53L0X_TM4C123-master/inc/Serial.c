 ...
 * You can find ValvanoWareTM4C123 at
 * http://edx-org-utaustinx.s3.amazonaws.com/UT601x/ValvanoWareTM4C123.zip?dl=1
 ...
 * Serial_Init
 * ----------
 * @brief Initialize the UART for 115,200 baud rate (assuming 80 MHz UART
...
void Serial_Init(void) {
  ...
  UART0_FBRD_R = 26;
  UART0_LCRH_R =
      (UART_LCRH_WLEN_8 | UART_LCRH_FEN);  // 8 bit word length (no parity bits,
                                           // one stop bit, FIFOs)
 ...
 * ----------
 * @warning If you enter a number above 4294967295, it will return an incorrect
 ...
 *          If you enter a number above FFFFFFFF, it will return an incorrect
 * value. Backspace will remove last digit typed.
 */
...
void Serial_putUHex(uint32_t number) {
  ...
  if (number >= 0x10) {
    ...
    if (number < 0xA)
      Serial_putChar(number + '0');
    else
      Serial_putChar((number - 0x0A) + 'A');
  }
}
