  ...
  OutCmd(0x04 + LCDINC + LCDNOSHIFT);  // I/D=1 Increment, S=0 no displayshift
                                       /* Display On/Off Control 0,0,0,0,1,D,C,B
                                            ...
                                            B  =1 for blink of cursor position character
                                               =0 for no blink   */
  OutCmd(0x0C + LCDNOCURSOR +
         LCDNOBLINK);  // D=1 displayon, C=0 cursoroff, B=0 blink off
...
int main(void) {
  uint32_t n;
  LCD_Init();  // leave system clock at 16 MHz, initialize LCD, initialize
               // SysTick, and initialize 74HC595 interface
  n = 0;
  LCD_Clear();
