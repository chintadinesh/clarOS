...
#define PINGPONGS \
  3  // number of DMA ping-pong transfers needed to send the whole encoding
     // buffer ((SIZE/PINGPONGS/2)<=1024 and an integer)
...
int main(void) {
  ...
  DMA_Init(32,
           (uint32_t *)&GPIO_PORTE0);  // initialize DMA channel 8 for Timer5A
