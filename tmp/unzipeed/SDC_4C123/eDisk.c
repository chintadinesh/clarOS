...
static void SELECT(void) { SDC_CS = 0; }

// de-asserts the CS pin to the card
static void DESELECT(void) { SDC_CS = 0xFF; }
...
static void rcvr_spi_m(BYTE *dst) { *dst = rcvr_spi(); }
...
static void power_off(void) { PowerFlag = 0; }
...
DSTATUS eDisk_Init(BYTE drv) { /* Physical drive nmuber (0) */
  ...
  if (send_cmd(CMD0, 0) == 1) {       /* Enter Idle state */
    ...
    if (send_cmd(CMD8, 0x1AA) == 1) { /* SDC Ver2+ */
      ...
      ty = (send_cmd(CMD55, 0) <= 1 && send_cmd(CMD41, 0) <= 1)
               ? 2
               : 1; /* SDC : MMC */
      do {
        if (ty == 2) {
...
#if _READONLY == 0
DRESULT eDisk_Write(BYTE drv,         /* Physical drive nmuber (0) */
