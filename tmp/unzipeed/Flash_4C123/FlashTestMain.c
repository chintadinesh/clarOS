...
#define FLASH \
  0x00008000  // location in flash to write; make sure no program code is in
              // this block
...
void printscores(void) {
  SBEType
      *Scoreboard;  // pointer to array of ScoreboardElements holding top scores
  int i;
...
void getnewscore(void) {
  ...
  while ((middle < 'A') || (('Z' < middle) && (middle < 'a')) ||
         ('z' < middle)) {
    if (middle >= ' ') {
      UART_OutChar(BS);  // backspace
    }
...
int main1(void) {
  ...
  LaunchPad_Init();    // activate clock for Port F
  ...
  Flash_Write(
      FLASH + 0x400,
      0x45464153);  // write to location 0x00008400; this value should persist
                    // after this program is run (see note at bottom of file)
  ...
  SuccessfulWrites =
      Flash_WriteArray(DataArray, FLASH + 9, 10);  // invalid address
  GPIO_PORTF321 = 0x04;  // red LED off; blue LED on; green LED off
  SuccessfulWrites =
      Flash_WriteArray(DataArray, FLASH + 12,
                       10);  // use scope to measure PF2 high time (678 usec)
  GPIO_PORTF321 = 0x00;      // red LED off; blue LED off; green LED off
  SuccessfulWrites =
      Flash_FastWrite(DataArray, FLASH + 124, 10);  // invalid address
  GPIO_PORTF321 = 0x02;  // red LED on; blue LED off; green LED off
  SuccessfulWrites =
      Flash_FastWrite(DataArray, FLASH + 128,
                      10);  // use scope to measure PF1 high time (335 usec)
  GPIO_PORTF321 = 0x00;     // red LED off; blue LED off; green LED off
  // memory test
  errors = 0;
  UART_OutString("\r\n");
  ...
  GPIO_PORTF321 = 0x04;  // red LED off; blue LED on; green LED off
                         /*  for(i=LTTSTART; i<LTTEND; i=i+1024){
                             Flash_Erase(i);                                 // clear the flash so long
                           term tests start at 0 errors; otherwise the first run will report errors
                           since flash already contains data
                           }*/
  while (1) {
    printscores();
    getnewscore();
