...
static uint32_t scoreblock =
    0x00000000;  // address of the first score saved in the scoreboard
static uint32_t *boardptr;  // pointer into the scoreboard; after processing,
                            // points to first empty location
static SBEType RAMScoreboard[SCOREBOARDSIZE];  // top scores stored in RAM in
                                               // descending order
...
// Output: pointer to an array of Scoreboard elements of length 'SCOREBOARDSIZE'
// (see Scoreboard.h)
SBEType *Scoreboard_Init(uint32_t addr) {
  ...
  if (AddrValid(addr)) {
    ...
    // [0x00]
    // [32-bit score]
    while (((initials & 0x000000FF) == 0x00000000) &&
           (boardptr <= (uint32_t *)(scoreblock + 0x3F8))) {
...
void Scoreboard_Record(char first, char middle, char last, uint32_t score) {
  ...
  if ((boardptr <= (uint32_t *)(scoreblock + 0x3F8))) {
    ...
    Flash_Write((uint32_t)boardptr,
                (first << 24) | (middle << 16) | (last << 8));
    ...
    for (i = 0; i < SCOREBOARDSIZE; i = i + 1) {
      Flash_Write((uint32_t)boardptr, (RAMScoreboard[i].first << 24) |
                                          (RAMScoreboard[i].middle << 16) |
                                          (RAMScoreboard[i].last << 8));
...
// Output: pointer to an array of Scoreboard elements of length 'SCOREBOARDSIZE'
// (see Scoreboard.h)
SBEType *Scoreboard_Get(void) { return RAMScoreboard; }
