...
// call the meta-sections.  The meta-sections are each a single int32_t that
// tells how many int32_ts/words can be stored between the header and trailer.
...
static int32_t splitAndMarkBlockUsed(int32_t* upperBlockStart,
                                     int32_t desiredRoom);
...
int32_t Heap_Test(void) {
  ...
  while (inHeapRange(blockStart)) {
    ...
    blockEnd = blockTrailer(blockStart);
    // error if blockEnd is not in the heap or blockend disagrees with
    // blockStart
...
heap_stats_t Heap_Stats(void) {
  ...
  stats.wordsOverhead =
      HEAP_SIZE_WORDS - stats.wordsAllocated - stats.wordsAvailable;
  return stats;
}
...
static int32_t blockUsed(int32_t* block) { return *block > 0; }

// blockUnused
// input: pointer to the header or trailer of a block
// output: whether or not the block is marked as unused/unallocated
static int32_t blockUnused(int32_t* block) { return *block < 0; }
...
// notes: given the header of the last block in the heap, will point to
// HEAP_END,
