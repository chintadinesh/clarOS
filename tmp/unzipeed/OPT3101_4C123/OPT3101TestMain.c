...
int main(void) {  // interrupt on completion, but continuous sampling as fast as
                  // possible
  uint32_t time = 0;
  DisableInterrupts();
  PLL_Init(Bus80MHz);
  ...
  while (1) {
    if (TxChannel <= 2) {  // 0,1,2 means new data
      TimeToConvert =
          ((StartTime - NVIC_ST_CURRENT_R) & 0x00FFFFFF) / 80;  // usec
      StartTime = NVIC_ST_CURRENT_R;
      if (TxChannel == 0) {
    ...
    if ((time & 0x7f) == 0) {  // show this every 128th sample (128*8=1024us)
      ST7735_SetCursor(6, 4);
      ST7735_OutUDec5(TimeToConvert);
      ST7735_SetCursor(13, 1);
      ST7735_OutUDec4(Noise5());
      ST7735_SetCursor(13, 2);
      ST7735_OutUDec4(Noise6());
      ST7735_SetCursor(13, 3);
      ST7735_OutUDec4(Noise7());
    }
    WaitForInterrupt();
  }
}
...
int main2(void) {  // busy-wait implementation
  ...
  while (1) {
    if (pollDistanceSensor()) {
      TimeToConvert =
          ((StartTime - NVIC_ST_CURRENT_R) & 0x00FFFFFF) / 80000;  // msec
      if (TxChannel <= 2) {
        ST7735_SetCursor(6, TxChannel + 1);
