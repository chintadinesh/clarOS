...
#define FLASH_FMC_WRKEY \
  0xA4420000  // FLASH write key (KEY bit of FLASH_BOOTCFG_R set)
#define FLASH_FMC_WRKEY2 \
  0x71D50000  // FLASH write key (KEY bit of FLASH_BOOTCFG_R cleared)
...
// Output: number of successful writes; return value == count if completely
// successful Note: at 80 MHz, it takes 678 usec to write 10 words Note:
...
int Flash_WriteArray(uint32_t *source, uint32_t addr, uint16_t count) {
  ...
  while ((successfulWrites < count) &&
         (Flash_Write(addr + 4 * successfulWrites, source[successfulWrites]) ==
          NOERROR)) {
...
// Output: number of successful writes; return value == count if completely
// successful Note: at 80 MHz, it takes 335 usec to write 10 words Note:
// disables interrupts while writing
