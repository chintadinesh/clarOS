...
void Timer0_Init(void) {
  ...
  TIMER0_CTL_R |= TIMER_CTL_TAEN;          // enable timer0A 16-b, +edge timing,
                                           // interrupts Timer0A=priority 2
...
void Timer0A_Handler(void) {
  ...
  if (ECHO0 == ECHO0HIGH) {          // first
    ...
    Timer0Pulse =
        (Timer0First - TIMER0_TAR_R) & 0xFFFFFF;  // 24 bits, 12.5ns resolution
    Timer0Done = 1;
  }
}
//-------------------------Timer1-------------------------------
...
void Timer1_Init(void) {
  ...
  TIMER1_CTL_R |= TIMER_CTL_TAEN;          // enable timer1A 16-b, +edge timing,
                                           // interrupts Timer0A=priority 2
...
void Timer1A_Handler(void) {
  ...
  if (ECHO1 == ECHO1HIGH) {          // first
    ...
    Timer1Pulse =
        (Timer1First - TIMER1_TAR_R) & 0xFFFFFF;  // 24 bits, 12.5ns resolution
    Timer1Done = 1;
  }
}
...
void Timer3_Init(void) {
  ...
  TIMER3_CTL_R |=
      TIMER_CTL_TAEN;  // enable timer3A 16-b, +edge timing, interrupts
...
void Timer3A_Handler(void) {
  ...
  if (ECHO3 == ECHO3HIGH) {          // first
    ...
    Timer3Pulse =
        (Timer3First - TIMER3_TAR_R) & 0xFFFFFF;  // 24 bits, 12.5ns resolution
    Timer3Done = 1;
  }
}
...
uint32_t Cycles2milliInch(uint32_t cycles) {
  // (x cycles)*(1 sec/80,000,000 cycles)*(13,560 in/sec)*(1,000 mil/1 in)*(0.5
  // round trip) = (x cycles)*6780)/BUS
...
uint32_t Cycles2millimeter(uint32_t cycles) {
  // (x cycles)*(1 sec/80,000,000 cycles)*(340.29 m/sec)*(1,000 mm/1 m)*(0.5
  // round trip) =
