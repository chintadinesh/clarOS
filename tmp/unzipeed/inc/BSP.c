// BSP.c
// Runs on TM4C123 with a Educational BoosterPack MKII (BOOSTXL-EDUMKII)
// This file contants the software interface to the MKII BoosterPack.
// This board support package (BSP) is an abstraction layer,
//   forming a bridge between the low-level hardware and the high-level
//   software.
...
// {TM4C123 PA6, MSP432 P6.5} J1.10 ambient light (OPT3001) and temperature
// sensor (TMP006) I2C SDA (I2C) {TM4C123 PA7, MSP432 P6.4}
//--------------------------------------------------
// J2.11 temperature sensor (TMP006) interrupt (digital) {TM4C123 PA2, MSP432
// P3.6} J2.12 nothing           {TM4C123 PA3, MSP432 P5.2} J2.13 LCD SPI CS
// (SPI)                {TM4C123 PA4, MSP432 P5.0} J2.14 nothing {TM4C123 PB6,
// MSP432 P1.7} J2.15 LCD SPI data (SPI)              {TM4C123 PB7, MSP432 P1.6}
// J2.16 nothing (reset)
...
// J4.35 Profile 6 {TM4C123 PC6, MSP432 P6.7}
...
// J4.39 RGB LED red (jumper up) or LCD backlight (jumper down) (PWM) {TM4C123
// PF3, MSP432 P2.6} J4.40 buzzer (PWM)                    {TM4C123 PF2, MSP432
// P2.7}
//--------------------------------------------------
...
// J1.9 ambient light (OPT3001) and temperature sensor (TMP006) I2C SCL (I2C)
// {TM4C123 PA6, MSP432 P6.5} J1.10 ambient light (OPT3001) and temperature
// sensor (TMP006) I2C SDA (I2C) {TM4C123 PA7, MSP432 P6.4} J2.11 temperature
// sensor (TMP006) interrupt (digital) {TM4C123 PA2, MSP432 P3.6}
//--------------------------------------------------
// Bluetooth booster
// J1.3 UART from Bluetooth to LaunchPad (UART){TM4C123 PB0, MSP432 P3.2}
// J1.4 UART from LaunchPad to Bluetooth (UART){TM4C123 PB1, MSP432 P3.3}
...
void static adcinit(void) {
  ...
  ADC0_SSPRI_R = 0x3210;  // 9) Sequencer 3 is lowest priority
                          // 10-15) sample sequencer initialization in more
                          // specific functions
}
...
void BSP_RGB_Init(uint16_t red, uint16_t green, uint16_t blue) {
  ...
  TIMER1_TBMATCHR_R =
      (red * PWMCycles) >>
      10;  // defines when output signal is cleared
  ...
  while ((SYSCTL_PRWTIMER_R & 0x01) == 0) {
  };                                 // allow time for clock to stabilize
...
void BSP_RGB_Set(uint16_t red, uint16_t green, uint16_t blue) {
  ...
  TIMER1_TBMATCHR_R =
      (red * PWMCycles) >> 10;  // defines when output signal is cleared
  TIMER3_TBMATCHR_R =
      (green * PWMCycles) >> 10;  // defines when output signal is cleared
  WTIMER0_TAMATCHR_R =
      (blue * PWMCycles) >> 10;  // defines when output signal is cleared
}
...
void BSP_RGB_D_Init(int red, int green, int blue) {
  ...
  while ((SYSCTL_PRGPIO_R & 0x26) != 0x26) {
  };                            // allow time for clocks to stabilize
...
void BSP_Buzzer_Set(uint16_t duty) {
  ...
  TIMER1_TAMATCHR_R =
      (duty * PWMCycles) >> 10;  // defines when output signal is cleared
}
...
void BSP_Accelerometer_Init(void) {
  ...
  GPIO_PORTD_AMSEL_R |= 0x07;  // 3) enable analog on PD2-0
                               // 4) configure PD2-0 as ?? (skip this line
                               // because PCTL is for digital only)
  ...
  ADC0_SSCTL2_R =
      0x0600;  // 13) no D0 END0 IE0 TS0 D1 END1 IE1 TS1 D2 TS2, yes IE2 END2
...
void BSP_Microphone_Init(void) {
  ...
  GPIO_PORTE_AMSEL_R |= 0x20;  // 3) enable analog on PE5
                               // 4) configure PE5 as ?? (skip this line because
...
void static commonInit(const uint8_t *cmdList) {
  ...
  SSI2_CR1_R &= ~SSI_CR1_MS;   // master mode
                               // configure for clock from source PIOSC for baud
                               // clock source
    ...
    for (j = 0; j < 8; j++) {
      if (line & 0x1) {
        if (size == 1)  // default size
          ...
          BSP_LCD_FillRect(x + (i * size), y + (j * size), size, size,
                           textColor);
        }
...
//        y         rows from the top edge (0 to 11)
...
//         newY  new Y-position of the cursor (0<=newY<=11)
// outputs: none
void BSP_LCD_SetCursor(uint32_t newX, uint32_t newY) {
  if ((newX > 20) || (newY > 11)) {  // bad input
    return;                          // do nothing
  }
  StX = newX;
  StY = newY;
...
int32_t Ymax, Ymin,
    Yrange;  // vertical axis max, min, and range (units not specified)
uint16_t PlotBGColor;  // background color of the plot used whenever clearing
                       // plot area
...
uint32_t BSP_Clock_GetFreq(void) { return ClockFrequency; }

// ------------BSP_PeriodicTask_Init------------
// Activate an interrupt to run a user task periodically.
// Give it a priority 0 to 7 with lower numbers
...
void BSP_PeriodicTask_Init(void (*task)(void), uint32_t freq,
                           uint8_t priority) {
  long sr;
  ...
  while ((SYSCTL_PRWTIMER_R & 0x20) == 0) {
  };                                 // allow time for clock to stabilize
  ...
  WTIMER5_CFG_R = TIMER_CFG_16_BIT;  // configure for 32-bit timer mode
                                     // configure for periodic mode, default
                                     // down-count settings
...
//  }                                         // repeat if error
//  while(((I2C1_MCS_R&(I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0) && (retryCounter <=
//  MAXRETRIES)); return (I2C1_MDR_R&0xFF);          // usually returns 0xFF on
//  error
//}
...
uint16_t static I2C_Recv2(int8_t slave) {
  ...
  while (((I2C1_MCS_R & (I2C_MCS_ADRACK | I2C_MCS_ERROR)) != 0) &&
...
// Returns 0 if successful, nonzero if error
uint16_t static I2C_Send3(int8_t slave, uint8_t data1, uint8_t data2,
                          uint8_t data3) {
  while (I2C1_MCS_R & I2C_MCS_BUSY) {
  };                                 // wait for I2C ready
  I2C1_MSA_R = (slave << 1) & 0xFE;  // MSA[7:1] is slave address
  ...
  I2C1_MCS_R =
      (0
       //                       & ~I2C_MCS_ACK     // no data ack (no data on
       //                       send) & ~I2C_MCS_STOP    // no stop
       | I2C_MCS_START  // generate start/restart
       | I2C_MCS_RUN);  // master enable
  while (I2C1_MCS_R & I2C_MCS_BUSY) {
  };  // wait for transmission done
  ...
  if ((I2C1_MCS_R & (I2C_MCS_DATACK | I2C_MCS_ADRACK | I2C_MCS_ERROR)) != 0) {
    I2C1_MCS_R =
        (0  // send stop if nonzero
            //                       & ~I2C_MCS_ACK     // no data ack (no data
            //                       on send)
         | I2C_MCS_STOP  // stop
         //                       & ~I2C_MCS_START   // no start/restart
         //                       & ~I2C_MCS_RUN     // master disable
  ...
  I2C1_MCS_R = (0
                //                       & ~I2C_MCS_ACK     // no data ack (no
                //                       data on send) & ~I2C_MCS_STOP    // no
                //                       stop & ~I2C_MCS_START   // no
                //                       start/restart
                | I2C_MCS_RUN);  // master enable
  while (I2C1_MCS_R & I2C_MCS_BUSY) {
  };  // wait for transmission done
...
void static lightsensorstart(uint8_t slaveAddress) {
  ...
  // 15-12 RN         range number         1100b = automatic full-scale setting
  // mode 11    CT         conversion time         1b = 800 ms 10-9  M mode of
  // conversion     01b = single-shot 8     OVF        overflow flag field 0b
  // (read only) 7     CRF        conversion ready field  0b (read only) 6 FH
  // flag high field         0b (read only) 5     FL         flag low field 0b
  // (read only) 4     L          latch                   1b = latch interrupt
  // if measurement exceeds programmed ranges 3     POL        polarity 0b = INT
  // pin reports active low 2     ME         mask exponent           0b = do not
  // mask exponent (more math later) 1-0   FC         fault count            00b
  // = 1 fault triggers interrupt
  I2C_Send3(slaveAddress, 0x01, 0xCA, 0x10);
  I2C_Recv2(
      slaveAddress);  // read Configuration Register to reset conversion ready
}
...
int32_t static lightsensorend(uint8_t slaveAddress) {
  ...
  // force the INT pin to clear by clearing and resetting the latch bit of the
  // Configuration Register (0x01)
  I2C_Send1(slaveAddress,
            0x01);  // pointer register 0x01 = Configuration Register
  config = I2C_Recv2(slaveAddress);  // current Configuration Register
  I2C_Send3(slaveAddress, 0x01, (config & 0xFF00) >> 8,
            (config & 0x00FF) & ~0x0010);
  I2C_Send3(slaveAddress, 0x01, (config & 0xFF00) >> 8,
            (config & 0x00FF) | 0x0010);
...
void static tempsensorstart(uint8_t slaveAddress) {
  ...
  // 14-12 MOD        mode of operation     111b = sensor and die continuous
  // conversion 11-9  CR         ADC conversion rate   010b = 1 sample/sec 8 EN
  // interrupt pin enable    1b = ~DRDY pin enabled (J2.11/P3.6) 7     ~DRDY
  // data ready bit          0b (read only, automatic clear) 6-0   reserved
  // 000000b (reserved)
...
void static tempsensorend(uint8_t slaveAddress, int32_t *sensorV,
                          int32_t *localT) {
  int16_t raw;
  I2C_Send1(slaveAddress,
            0x00);  // pointer register 0x00 = Sensor Voltage Register
  raw = I2C_Recv2(slaveAddress);
  *sensorV = raw * 15625;  // 156.26 nV per LSB
  I2C_Send1(slaveAddress,
            0x01);  // pointer register 0x01 = Local Temperature Register
  raw = I2C_Recv2(slaveAddress);
