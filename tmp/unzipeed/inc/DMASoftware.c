...
void DMA_Init(void) {
  ...
  UDMA_REQMASKCLR_R = BIT30;  // allow the µDMA controller to recognize requests
                              // for this channel
}
// ************DMA_Transfer*****************
...
void DMA_Transfer(uint32_t *source, uint32_t *destination, uint32_t count) {
  ...
  ucControlTable[CH30 + 1] =
      (uint32_t)destination + count * 4 - 1;  // last address
  ucControlTable[CH30 + 2] =
      0xAA00C002 + ((count - 1) << 4);  // DMA Channel Control Word (DMACHCTL)
  ...
  // vector 62, NVIC interrupt 46, vector address 0x0000.00F8 could be armed or
...
uint32_t DMA_Status(void) {
  return (UDMA_ENASET_R &
          BIT30);  // µDMA Channel 30 enable nit is high if active
}
