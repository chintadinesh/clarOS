...
static const uint32_t g_pulRegs[] = {
    ...
    NVIC_PRI11, NVIC_PRI12,    NVIC_PRI13};
...
#if defined(ewarm)
static __no_init void (*g_pfnRAMVectors[NUM_INTERRUPTS_TM4C123])(
    void) @ "VTABLE";
#elif defined(sourcerygxx)
static __attribute__((section(".cs3.region-head.ram"))) void (
    *g_pfnRAMVectors[NUM_INTERRUPTS_TM4C123])(void);
#elif defined(ccs)
...
//
//*****************************************************************************
int32_t IntPriorityGet(uint32_t ulInterrupt) {
  //
  // Check the arguments.
  //
  ASSERT((ulInterrupt >= 4) && (ulInterrupt < NUM_INTERRUPTS_TM4C123));
