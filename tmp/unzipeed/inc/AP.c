...
// CC2650 booster or CC2650 LaunchPad, CC2650 needs to be running SimpleNP 2.2
// (POWERSAVE) UART serial from CC2650 to TM4C123/MSP432 UART serial from
// TM4C123/MSP432 to CC2650 MRDY, an output from TM4C123/MSP432 and an input to
// CC2650 SRDY, an input to TM4C123/MSP432 and an output from CC2650 Reset, an
// output from TM4C123/MSP432 to hardware reset to CC2650 MRDY is a GPIO pin
// whose logic level indicates the state of the master processor. MRDY is an
// active low (low true) signal. An MRDY event indicates that the master
// processor is ready to send or receive data. SRDY is a GPIO pin whose logic
// level indicates the state of the slave processor. SRDY is an active low (low
// true) signal. An SRDY event indicates that the slave processor is ready to
// send or receive data. see GPIO.c file for hardware connections
...
/* This example accompanies the books
 ...
 Microcontrollers", ISBN: 978-1466468863, , Jonathan Valvano, copyright (c) 2016
...
const uint8_t HCI_EXT_ResetSystemCmd[] = {SOF,  0x03, 0x00, 0x55, 0x04,
                                          0x1D, 0xFC, 0x01, 0xB2};
...
uint8_t NPI_AddService[] = {
    ...
    0xF0, 0xFF, 0xB9};  // FCS (calculated by AP_SendMessageResponse)
...
uint8_t NPI_GATTSetDeviceName[] = {
    ...
    'S',  'h',  'a',  'p', 'e', ' ', 't', 'h', 'e', ' ', 'W',
    'o',  'r',  'l',  'd', ' ', '0', '0', '1', 0x77};  // FCS (calculated by
...
const uint8_t NPI_StartAdvertisement[] = {
    ...
    0x02,   // Advertising will restart with connectable advertising when a
...
uint8_t NPI_ReadConfirmation[] = {
    ...
    0x00, 0x00,       // Handle of the characteristic value attribute being read
                      // (filled in dynamically
    0x00, 0x00,       // offset, ignored, assumes small chucks of data
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00,   // actual data (filled in dynamically)
    0x00};  // FCS (calculated by AP_SendMessageResponse)
...
uint8_t NPI_SendNotificationIndication[] = {
    ...
    0x00, 0x00,       // handle of connection always 0
    0x00, 0x00,  // Handle of the characteristic value attribute to notify /
                 // indicate (filled in dynamically
    0x00,        // RFU
...
uint8_t NPI_AddCharDescriptor[] = {
    ...
    'C',  'h',  'a',  'r', 'a', 'c', 't', 'e', 'r',
    'i',  's',  't',  'i', 'c', ' ', '0', 0,  // Initial user description string
...
int AP_Init(void) {
  ...
  while (bwaiting) {
    AP_Reset();
    count = 0;  // should get SNP power up within 30 ms (duration is arbitrary
                // and 'count' value is uncalibrated)
    while (count < 600000) {
      if (AP_RecvStatus()) {
  ...
  count = 0;     // should get SNP power up within 120 ms (duration is arbitrary
                 // and 'count' value is uncalibrated)
...
uint32_t AP_RecvStatus(void) { return (ReadSRDY() == 0); }
...
// this does not perform BLU communication, it reads current CCCD value of
// notify characteristic Inputs:  index into which notify characteristic to
// return Outputs: 16-bit CCCD value of the notify characteristic
...
//        (*ReadFunc) called before it responses with data from internal
//        structure
//        (*WriteFunc) called after it accepts data into internal structure
// Output APOK if successful,
//        APFAIL if name is empty, more than 8 characteristics, or if SNP
//        failure
int AP_AddCharacteristic(uint16_t uuid, uint16_t thesize, void *pt,
                         uint8_t permission, uint8_t properties, char name[],
                         void (*ReadFunc)(void), void (*WriteFunc)(void)) {
  int r;
  uint16_t handle;
  int i;
  ...
  NPI_AddCharValue[6] =
      properties;  // 2=read,8=write,0x0A=read+write,0x10=notify,
                   // GATT Properties
  NPI_AddCharValue[11] = 0xFF & uuid;
  ...
  NPI_AddCharDescriptor[3] =
      0x35;  // SNP Add Characteristic Descriptor Declaration
  ...
  r = AP_SendMessageResponse((uint8_t *)NPI_AddCharDescriptor, RecvBuf,
                             RECVSIZE);
  if (r == APFAIL) return APFAIL;
...
//        for read, write, or read/write characteristic, call
//        AP_AddCharacteristic
// Inputs uuid is 0xFFF0, 0xFFF1, ...
//        thesize is the number of bytes in the user data 1,2,4, or 8
//        pt is a pointer to the user data, stored little endian
...
// Output APOK if successful,
//        APFAIL if name is empty, more than 4 notify characteristics, or if SNP
//        failure
int AP_AddNotifyCharacteristic(uint16_t uuid, uint16_t thesize, void *pt,
  ...
  i = 0;
  while ((i < 19) && (name[i])) {
    NPI_AddCharDescriptor[12 + i] = name[i];
    i++;
  }
  if (i == 0) return APFAIL;  // empty name
  NPI_AddCharDescriptor[12 + i] = 0;
  i++;                               // add null termination
  NPI_AddCharDescriptor[1] = 7 + i;  // frame length
  NPI_AddCharDescriptor[3] =
      0x35;  // SNP Add Characteristic Descriptor Declaration
  NPI_AddCharDescriptor[4] = 0x83;
  NPI_AddCharDescriptor[5] =
      0x84;  // User Description String, and CCCD permissions
  ...
  NotifyCharacteristicList[NotifyCharacteristicCount].CCCDhandle =
      (RecvBuf[8] << 8) + RecvBuf[7];  // handle for this CCCD
  NotifyCharacteristicList[NotifyCharacteristicCount].CCCDvalue =
      0;  // notify initially off
...
int AP_SendNotification(uint32_t i) {
  ...
  if (NotifyCharacteristicList[i].CCCDvalue) {        // send only if active
    ...
    NPI_SendNotificationIndication[1] =
        6 + NotifyCharacteristicList[i].size;  // 1 to 8 bytes
    OutString("\n\rSend data=");
    s = NotifyCharacteristicList[i].size;
    for (j = 0; j < s; j++) {
      thedata =
          NotifyCharacteristicList[i]
              .pt[s - j -
                  1];  // fetch data from user little endian to SNP big endian
      OutUHex(thedata);
      OutString(", ");
      NPI_SendNotificationIndication[11 + j] =
          thedata;  // copy into message, big endian
    }
    NPI_SendNotificationIndication[7] = handle & 0x0FF;  // handle
    NPI_SendNotificationIndication[8] = handle >> 8;
    r1 = AP_SendMessageResponse(NPI_SendNotificationIndication, RecvBuf,
                                RECVSIZE);
  } else {
...
int AP_StartAdvertisement(void) {
  volatile int r;
  OutString("\n\rSet Device name");
  r = AP_SendMessageResponse((uint8_t *)NPI_GATTSetDeviceName, RecvBuf,
                             RECVSIZE);
  OutString("\n\rSetAdvertisement1");
  r = AP_SendMessageResponse((uint8_t *)NPI_SetAdvertisement1, RecvBuf,
                             RECVSIZE);
  //  OutString("\n\rSetAdvertisementSAP");
  //  r
  //  =AP_SendMessageResponse((uint8_t*)NPI_SetAdvertisementSAP,RecvBuf,RECVSIZE);
  OutString("\n\rSetAdvertisement Data");
  r = AP_SendMessageResponse((uint8_t *)NPI_SetAdvertisementData, RecvBuf,
                             RECVSIZE);
  OutString("\n\rStartAdvertisement");
  r = AP_SendMessageResponse((uint8_t *)NPI_StartAdvertisement, RecvBuf,
                             RECVSIZE);
  return r;
}
//*************AP_GetStatus**************
...
uint32_t AP_GetStatus(void) {
  ...
  return (RecvBuf[4] << 24) + (RecvBuf[5] << 16) + (RecvBuf[6] << 8) +
         (RecvBuf[7]);
}
//*************AP_GetVersion**************
...
void AP_BackgroundProcess(void) {
  ...
  if (AP_RecvStatus()) {
    if (AP_RecvMessage(RecvBuf, RECVSIZE) == APOK) {
      ...
      if ((RecvBuf[3] == 0x55) &&
          (RecvBuf[4] == 0x88)) {  // SNP Characteristic Write Indication (0x88)
        ...
        while (i < MAXCHARACTERISTICS) {
          if (CharacteristicList[i].theHandle == h) {
            ...
            (*CharacteristicList[i]
                  .callBackWrite)();  // process Characteristic Write Indication
      ...
      if ((RecvBuf[3] == 0x55) &&
          (RecvBuf[4] == 0x87)) {  // SNP Characteristic Read Indication (0x87)
        h = (RecvBuf[8] << 8) + RecvBuf[7];  // handle for this characteristic
        i = 0;
        while (i < MAXCHARACTERISTICS) {
          if (CharacteristicList[i].theHandle == h) {
            (*CharacteristicList[i]
                  .callBackRead)();  // process Characteristic Read Indication
            NPI_ReadConfirmation[1] = 7 + CharacteristicList[i].size;
            s = CharacteristicList[i].size;
            for (j = 0; j < s; j++) {  // write data
              NPI_ReadConfirmation[j + 12] =
                  CharacteristicList[i].pt[s - j - 1];
            }
            i = MAXCHARACTERISTICS;
          } else {
            i++;
      ...
      if ((RecvBuf[3] == 0x55) &&
          (RecvBuf[4] == 0x8B)) {  // SNP CCCD Updated Indication (0x8B)
        h = (RecvBuf[8] << 8) + RecvBuf[7];  // handle for this characteristic
        responseNeeded = RecvBuf[9];
        for (i = 0; i < NOTIFYMAXCHARACTERISTICS; i++) {
          if (NotifyCharacteristicList[i].CCCDhandle == h) {  // to do
            NotifyCharacteristicList[i].CCCDvalue =
                (RecvBuf[11] << 8) + RecvBuf[10];
