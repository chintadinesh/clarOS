...
//   return buffer[0] + ((uint32_t)buffer[1] << 8) + ((uint32_t)buffer[2] <<
//   16);
// }
void OPT3101_WriteRegister(uint8_t address, uint32_t data) {
  I2C0_Send4(I2C_ADDRESS, address, data & 0xFF, (data >> 8) & 0xFF,
             (data >> 16) & 0xFF);
}
// void OPT3101_WriteRegister(uint8_t address, uint32_t data){
//   uint8_t buffer[] = {
...
void OPT3101_Setup(void) {
  ...
  // every sub-frame.
  OPT3101_WriteRegister(
      0x9f, (uint32_t)(SUB_FRAME_COUNT - 1) << 12 | (SUB_FRAME_COUNT - 1));
...
void OPT3101_StartMeasurementChannel(uint32_t ch) {
  if (ch <= 2) {
    ...
    OPT3101_WriteRegister(
        0x2a, reg2a);  // Set MONOSHOT_BIT to 1 to trigger a new measurement.
  }
  // Assumption: The other bits in register 0 should be 0.
  OPT3101_WriteRegister(0x00, 0x800000);
}
...
// Returns true if we have a new distance measurement ready to be read, false
// otherwise.
