...
void UART1_Init(uint32_t baud) {
  ...
  // int(43.403) UART1_FBRD_R = 26;       // FBRD = round(0.4028 * 64 ) = 26
  UART1_IBRD_R = 5000000 / baud;  // IBRD = int(80,000,000 / (16 * baud))
  UART1_FBRD_R = ((64 * (5000000 % baud)) + baud / 2) /
                 baud;  // FBRD = round(remainder * 64 + 0.5)
  ...
  UART1_IM_R |= UART_IM_RXIM |
                UART_IM_RTIM;  // Enable interupt on RX and RX transmission end
  UART1_CTL_R |= UART_CTL_UARTEN;  // Set UART1 enable bit

  // UART1_DMACTL_R |= 0x01;
  // // Enable DMA for RX FIFO
