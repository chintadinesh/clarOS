...
void I2C1_Init(uint32_t I2Cfreq, uint32_t busFreq) {
  ...
  //  I2C1_MCR_R = I2C_MCR_MFE|I2C_MCR_GFE;             // 9) master function
  //  enable, glitch I2C1_MCR2_R = I2C_MCR2_GFPW_4; // 4 clock glitch
...
int I2C1_Send4(uint8_t slaveAddr, uint8_t data1, uint8_t data2, uint8_t data3,
               uint8_t data4) {
  while (I2C1_MCS_R & I2C_MCS_BUSY) {
  };                              // wait for I2C ready
  I2C1_MSA_R = (slaveAddr << 1);  // MSA[7:1] is slave address
...
uint16_t I2C1_Recv2(int8_t slave) {
  ...
  do {
    ...
    I2C1_MSA_R = (slave << 1) & 0xFE;  // MSA[7:1] is slave address
    I2C1_MSA_R |= 0x01;                // MSA[0] is 1 for receive
    I2C1_MCS_R = (0 | I2C_MCS_ACK      // positive data ack
                                       //  & ~I2C_MCS_STOP    // no stop
                  | I2C_MCS_START      // generate start/restart
                  | I2C_MCS_RUN);      // master enable
    while (I2C1_MCS_R & I2C_MCS_BUSY) {
