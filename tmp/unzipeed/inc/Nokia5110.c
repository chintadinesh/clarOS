...
// ~80mA total Ground        (Gnd, pin 8) ground
...
void Nokia5110_Init(void) {
  ...
  SSI0_CPSR_R =
      (SSI0_CPSR_R & ~SSI_CPSR_CPSDVSR_M) + 24;  // must be even number
  ...
  lcdwrite(COMMAND, 0x21);  // chip active; horizontal addressing mode (V = 0);
                            // use extended instruction set (H = 1) set LCD Vop
                            // (contrast), which may require some tweaking:
  lcdwrite(COMMAND,
           CONTRAST);  // try 0xB1 (for 3.3V red SparkFun), 0xB8 (for 3.3V blue
                       // SparkFun), 0xBF if your display is too dark, or 0x80
                       // to 0xFF if experimenting
  ...
  lcdwrite(COMMAND,
           0x0C);  // set display control to normal mode: 0x0D for inverse
}
...
uint8_t Screen[SCREENW * SCREENH /
...
// inputs: xpos      horizontal position of bottom left corner of image, columns
// from the left edge
//                     must be less than 84
//                     0 is on the left; 82 is near the right
//         ypos      vertical position of bottom left corner of image, rows from
//         the top edge
//                     must be less than 48
//                     2 is near the top; 47 is at the bottom
//         ptr       pointer to a 16 color BMP image
//         threshold grayscale colors above this number make corresponding pixel
//         'on'
//                     0 to 14
...
// outputs: none
// assumes: LCD is in default horizontal addressing mode (V = 0)
void Nokia5110_DisplayBuffer(void) { Nokia5110_DrawFullImage(Screen); }
...
int Nokia_close(int dev_fd) { return 0; }
int Nokia_read(int dev_fd, char *buf, unsigned count) {  // char ch;
  return 0;
}
int Nokia_write(int dev_fd, const char *buf, unsigned count) {
...
off_t Nokia_lseek(int dev_fd, off_t ioffset, int origin) { return 0; }
int Nokia_unlink(const char *path) { return 0; }
int Nokia_rename(const char *old_name, const char *new_name) { return 0; }
...
void Output_Init(void) {
  ...
  Nokia5110_Init();
  NokiaLineNumber = 0;
  ret_val = add_device("Nokia", _SSA, Nokia_open, Nokia_close, Nokia_read,
                       Nokia_write, Nokia_lseek, Nokia_unlink, Nokia_rename);
