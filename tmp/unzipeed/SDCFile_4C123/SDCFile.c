...
void FileSystemTest(void) {
  ...
  if (Fresult) {
    ...
    if (Fresult) {
      ST7735_DrawString(0, 0, "file2 f_close error",
                        ST7735_Color565(0, 0, 255));
      while (1) {
      };
    }
  }
  ...
  if (Fresult == FR_OK) {
    ...
    ST7735_DrawString(0, 0, "Error testFile.txt (  )",
                      ST7735_Color565(255, 0, 0));
  ...
  ST7735_DrawString(0, 0, "file test passed    ",
...
int main(void) {
  ...
  EnableInterrupts();
  //  SimpleUnformattedTest();              // comment this out if continuing to
  //  the advanced file system tests
  ...
  if (Fresult == FR_OK) {
    ...
    // get a character in 'c' and the number of successful reads in
    // 'successfulreads'
    Fresult = f_read(&Handle, &c, 1, &successfulreads);
  ...
  // Options:
  // FA_CREATE_NEW    - Creates a new file, only if it does not already exist.
  // If file already exists, the function fails. FA_CREATE_ALWAYS - Creates a
  // new file, always.  If file already exists, it is over-written.
  // FA_OPEN_ALWAYS   - Opens a file, always.  If file does not exist, the
  // function creates a file. FA_OPEN_EXISTING - Opens a file, only if it
  // exists.  If the file does not exist, the function fails.
  ...
  if (Fresult == FR_OK) {
    ...
    // write a message and get the number of successful writes in
    // 'successfulwrites'
    ...
    if (Fresult == FR_OK) {
      ...
      // print the byte offset from the start of the file where the writes
      // started expect: (third parameter of f_write())*(number of times this
      // program has been run before)
      ST7735_OutUDec((uint32_t)(Handle.fptr - successfulwrites));
    } else {
      // print the error code
      ST7735_DrawString(0, 15, "f_write() error (  )",
                        ST7735_Color565(255, 0, 0));
