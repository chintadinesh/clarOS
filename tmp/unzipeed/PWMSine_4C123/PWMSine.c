...
// PF2 toggles on each timer interrupt (flash at frequency of sine wave * 32 /
// 2)
...
const uint16_t Wave[32] = {125, 143, 159, 175, 189, 200, 208, 213,
                           215, 213, 208, 200, 189, 175, 159, 143,
                           125, 107, 91,  75,  61,  50,  42,  37,
                           35,  37,  42,  50,  61,  75,  91,  107};
...
int main(void) {
  ...
  Timer0A_Init(&OutputSineWave, 1563,
               1);  // initialize 1000 Hz sine wave output
  while (1) {
    WaitForInterrupt();
  }
}
