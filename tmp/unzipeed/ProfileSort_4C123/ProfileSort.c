  ...
  for (j = 1; j < length; j = j + 1) {
    ...
    for (i = j; i > 0; i = i - 1) {
      if (output[i] < output[i - 1]) {
        ...
        // A is the number of times 'i' decreases to 0 (number of 2's
        // immediately followed by 1's without a 3 in between) B is the number
        // of moves (number of 2's in Debug_place)
#endif
#if (DEBUGTYPE == HWPORT)
        PROFILE = 8;
#endif
...
// Numerical keys associated with data from "The Art of Computer Programming",
// Donald E. Knuth, Volume 3 Sorting and Searching, Second Edition, 1998,
// page 77.
uint32_t keys[16] = {503, 87,  512, 61,  908, 170, 897, 275,
...
int main(void) {
  ...
  while ((i < DEBUG_LENGTH) && (Debug_place[i] != 0)) {
    if (Debug_place[i] == 2) {
      KnuthB = KnuthB + 1;
      if (Debug_place[i + 1] == 1) {  // possible array out-of-bounds error if
                                      // DEBUG_LENGTH is too small
        KnuthA = KnuthA + 1;
      }
    }
    i = i + 1;
  ...
  KnuthRunTime =
      9 * KnuthB + 10 * KnuthN - 3 * KnuthA - 9;  // expect 514 for his data
#endif
  // *********************************************************
