...
// I2C0SCL connected to PB2 and to pin 4 of HMC6352 compass or pin 3 of TMP102
// thermometer I2C0SDA connected to PB3 and to pin 3 of HMC6352 compass or pin 2
// of TMP102 thermometer SCL and SDA lines pulled to +3.3 V with 10 k resistors
// (part of breakout module) ADD0 pin of TMP102 thermometer connected to GND
#include <stdint.h>
  ...
  if (rawData & 0x0001) {  // 13-bit mode
    *tempInt = rawData >> 7;
    if (rawData & 0x8000) {  // negative value
      ...
      *tempInt = (*tempInt) - 1512;  // subtract 512 to make integer portion
                                     // signed subtract extra 1,000 so integer
                                     // portion is still negative in the case of
                                     // -0.XXXX (never display thousands digit)
      if (((*tempFra) > 0) &&
          (*tempFra) < 10000) {  // fractional part "borrows" from integer part
        *tempInt = (*tempInt) + 1;
      }
    } else {
    ...
    *tempInt = rawData >> 8;
    if (rawData & 0x8000) {  // negative value
      ...
      *tempInt = (*tempInt) - 1256;  // subtract 256 to make integer portion
                                     // signed subtract extra 1,000 so integer
                                     // portion is still negative in the case of
                                     // -0.XXXX (never display thousands digit)
      if (((*tempFra) > 0) &&
          (*tempFra) < 10000) {  // decimal part "borrows" from integer part
        *tempInt = (*tempInt) + 1;
      }
    } else {
...
int main(void) {
  ...
  I2C_Send1(0x48, 1);  // use command 1 to set pointer to config (Figure 7.XX
                       // chapter7-10-1.ulb) read from 0x48 to get data
  if (I2C_Recv2(0x48) == 0x60A0) {  // expected 0x60A0 as power-on default
                                    // (Figure 7.XX chapter7-10-2.ulb)
    UART_OutString("\r\nTest Passed\r\n");
  } else {
...
// test display and number parser (can safely be skipped)
  ...
  Clock_Delay(DEBUGWAIT);                 // short delay
  ...
  while (1) {
    ...
    rawData = I2C_Recv2(
        0x48);  // temperature data stored in 12 or 13 MSBs (Figure 7.XX
                // chapter7-10-3.ulb, reply 0x1660, 22.375 C)
    parseTemp(rawData, &tempInt, &tempFra);
    displayTemp(&tempInt, &tempFra);
    Clock_Delay(4166667);  // wait 250 ms to sample at ~4 Hz
  }
  /*  while(1){
                                            // write commands to 0x42
      I2C_Send1(0x42, 'A');                 // use command 'A' to sample
      Clock_Delay(100000);                  // wait 6,000 us for sampling to
  finish
  ...
  // test sending multiple bytes and receiving single byte
                                            // write commands to 0x42
      I2C_Send2(0x42, 'g', 0x74);           // use command 'g' to read from RAM
  0x74 Clock_Delay(1167);                    // wait 70 us for RAM access to
  finish controlReg = I2C_Recv(0x43);          // expected 0x50 as default
      Clock_Delay(16666666);                // wait 1 sec
    }*/
}
