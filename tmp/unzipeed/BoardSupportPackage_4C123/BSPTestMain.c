...
// ambient light (OPT3001) and temperature sensor (TMP006) I2C SCL connected to
// J1.9 (I2C) ambient light (OPT3001) and temperature sensor (TMP006) I2C SDA
// connected to J1.10 (I2C)
...
// RGB LED red (jumper up) or LCD backlight (jumper down) connected to J4.39
// (PWM) buzzer connected to J4.40 (PWM)
...
void checkbuttons(void) {
  ...
  if (mode == 0) {
    ...
    // accelerometer mode
    if ((AccX < 325) && (AccY > 325) && (AccY < 675) && (AccZ > 325) &&
        (AccZ < 675)) {
      Red = 500;
      Green = 0;
      Blue = 0;
    } else if ((AccY < 325) && (AccX > 325) && (AccX < 675) && (AccZ > 325) &&
               (AccZ < 675)) {
      Red = 350;
      Green = 350;
      Blue = 0;
    } else if ((AccX > 675) && (AccY > 325) && (AccY < 675) && (AccZ > 325) &&
               (AccZ < 675)) {
      Red = 0;
      Green = 500;
      Blue = 0;
    } else if ((AccY > 675) && (AccX > 325) && (AccX < 675) && (AccZ > 325) &&
               (AccZ < 675)) {
      Red = 0;
      Green = 0;
      Blue = 500;
...
int main(void) {
  ...
  while (1) {
    ...
    if (count == 10) {
      ...
      BSP_LCD_DrawString(0, 8,
                         "Light=        ", BSP_LCD_Color565(255, 255, 255));
      ...
      if (numlength(light % 100) == 1) {
        // need to print a leading zero, which BSP_LCD_OutUDec() does not do
        // automatically so 1/100 prints as ".01" instead of ".1 "
      ...
      BSP_LCD_DrawString(0, 9,
                         "Volt=           ", BSP_LCD_Color565(255, 255, 255));
      if (voltage < 0) {
        ...
        BSP_LCD_DrawString(6 + numlength((-1 * voltage) / 100000000), 9, ".",
                           color);
        BSP_LCD_SetCursor(7 + numlength((-1 * voltage) / 100000000), 9);
        for (i = numlength((-1 * voltage) % 100000000); i < 8; i = i + 1) {
          // print any leading zeroes, which BSP_LCD_OutUDec() does not do
          // automatically so 1/100,000,000 prints as ".00000001" instead of ".1
          // "
          BSP_LCD_OutUDec(0, color);
        }
        BSP_LCD_OutUDec((-1 * voltage) % 100000000, color);
      ...
      BSP_LCD_DrawString(0, 11, "Time=  :  .      ",
                         BSP_LCD_Color565(255, 255, 255));
      ...
      if (numlength((time % 60000000) / 1000000) == 1) {
        // print a leading zero, which BSP_LCD_OutUDec() does not do
        // automatically so 1/60 prints as ":01" instead of ":1 "
        BSP_LCD_OutUDec(0, color);
      }
      ...
      for (i = numlength(time % 1000000); i < 6; i = i + 1) {
        // print any leading zeroes, which BSP_LCD_OutUDec() does not do
        // automatically so 1/1,000,000 prints as ":000001" instead of ".1 "
        BSP_LCD_OutUDec(0, color);
      }
