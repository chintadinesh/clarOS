...
//              <2=> 2: IOSC/4 Internal oscillator / 4 (this is necessary if
//              used as input to PLL) <3=> 3: 30kHz 30-KHz internal oscillator
...
//
#define RCC_Val                                                           \
  (((CFG_RCC_SYSDIV - 1) << 23) | (CFG_RCC_USESYSDIV << 22) |             \
   (CFG_RCC_USEPWMDIV << 20) | (CFG_RCC_PWMDIV << 17) |                   \
   (CFG_RCC_PWRDN << 13) | (CFG_RCC_BYPASS << 11) | (CFG_RCC_XTAL << 6) | \
   (CFG_RCC_OSCSRC << 4) | (CFG_RCC_IOSCDIS << 1) | (CFG_RCC_MOSCDIS << 1))

#define RCC2_Val                                              \
  ((CFG_RCC2_USERCC2 << 31) | ((CFG_RCC_SYSDIV2 - 1) << 23) | \
   (CFG_RCC_PWRDN2 << 13) | (CFG_RCC_BYPASS2 << 11) | (CFG_RCC_OSCSRC2 << 4))
...
uint32_t SystemCoreClock =
    __CORE_CLK; /*!< System Clock Frequency (Core Clock)*/
...
/* FPU settings ------------------------------------------------------------*/
  ...
  SYSCTL->RCC =
      (RCC_Val | (1UL << 11) | (1UL << 13)) &
      ~(1UL << 22); /* set value with BYPASS, PWRDN set, USESYSDIV reset */
  SYSCTL->RCC2 = (RCC2_Val | (1UL << 11) |
                  (1UL << 13)); /* set value with BYPASS, PWRDN set */
  for (i = 0; i < 1000; i++)
    ; /* wait a while */

  SYSCTL->RCC = (RCC_Val | (1UL << 11)) &
                ~(1UL << 22); /* set value with BYPASS, USESYSDIV reset */
