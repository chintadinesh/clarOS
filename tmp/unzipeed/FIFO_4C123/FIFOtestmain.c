...
#define INTPERIOD 1100    // interrupt period (80MHz cycles)
#define INTVARIATION \
  300  // maximum interrupt period variation from "INTPERIOD" (50MHz cycles)
...
// return the previous PC after an interrupt has occurred
// assuming floating point not active and assuming UserTask has been called
...
#else
                    // Keil uVision Code
__asm uint32_t Who(void) {
  LDR R0, [ SP, #48 ];
  R0 = return address from interrupt bx LR
}
#endif
...
void UserTask(void) {
  ...
  returnaddress = Who();
  if ((returnaddress < (uint32_t)&RxFifo_Get) ||
...
int main(void) {
  ...
  Timer0A_Init(
      &UserTask, INTPERIOD,
      2);  // start value to count down from
           // *************** Test #5: test interrupt vulnerability ************
  RxFifo_Init();
  for (i = 0; i < HISTOGRAMSIZE; i = i + 1) {
    uint32_t returnaddress;
    LineHistogram[i] = 0;
    returnaddress = ((uint32_t)&RxFifo_Get + 2 * i) & 0xFFFFFFFE;
    LineHistogramAddress[i] =
        returnaddress;  // possible places in Get that could be interrupted
  }
  EnableInterrupts();
  ...
  while (1) {
    ...
    if (ForeExpected != ForeActual) {
      ...
      ForeExpected =
          ForeExpected + 1;  // sequence is 0,1,2,3,...,254,255,0,1,...
    }
    PF2 = 0;
  }
}
