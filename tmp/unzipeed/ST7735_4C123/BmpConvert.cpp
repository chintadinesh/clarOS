...
void main(void) {
  ...
  FILE *in, *out;
  printf(
      "This program converts a 24-bit color BMP into C source code for ST7735 "
      "displays running in 16-bit color mode\n");
  printf("Enter file: ");
  scanf("%s", BMPname);
  ...
  // The next four bytes are application specific and are ignored in this
  // context.
  fgetc(in);
  fgetc(in);
  fgetc(in);
  fgetc(in);
  // The next four bytes are the offset where the data can be found after the
  // header.
  bmpDataOff = fgetc(in);
  ...
  // The next four bytes are the number of bytes in the DIB header, including
  // these four.
  bmpDIBSize = fgetc(in);
  ...
  if ((bmpDataOff - bmpDIBSize) != 14) {
    fprintf(
        stderr,
        "Warning: This file's data offset is %d bytes (expected %d bytes).\n",
        bmpDataOff, (bmpDIBSize + 14));
  }
  if (bmpDIBSize > 40) {
    fprintf(stderr,
            "Warning: This file's header is %d bytes (expected 40 bytes), "
            "which may result in other errors.\n",
            bmpDIBSize);
  }
  if (bmpDIBSize < 40) {
    fprintf(stderr,
            "Error: This file's header is %d bytes (expected 40 bytes).\n",
            bmpDIBSize);
    fclose(in);
    fclose(out);
    scanf("%d", &data);
  ...
  if (bmpWidth > 128) {
    fprintf(stderr,
            "Warning: This image's width of %d exceeds the ST7735's 128 pixel "
            "width.\n",
            bmpWidth);
  }
  ...
  if (bmpHeight > 160) {
    fprintf(stderr,
            "Warning: This image's height of %d exceeds the ST7735's 160 pixel "
            "height.\n",
            bmpHeight);
  }
  if (bmpHeight < 0) {
    fprintf(stderr,
            "Warning: This image is encoded in top-to-bottom pixel order, a "
            "format not currently supported by this converter.\n");
  }
  // The next two bytes are the number of color panes and are ignored in this
  // context.
  fgetc(in);
  fgetc(in);
  ...
  if (bmpPxlBits != 24) {
    fprintf(stderr,
            "Error: This file is not a bitmap with 24 bits per pixel.\n");
    fclose(in);
    fclose(out);
    scanf("%d", &data);
    return;
  }
  // The next four bytes are related to pixel array compression and are ignored
  // in this context.
  fgetc(in);
  fgetc(in);
  fgetc(in);
  ...
  // The next four bytes are the horizontal resolution of the image and are
  // ignored in this context.
  fgetc(in);
  fgetc(in);
  fgetc(in);
  fgetc(in);
  // The next four bytes are the vertical resolution of the image and are
  // ignored in this context.
  fgetc(in);
  fgetc(in);
  fgetc(in);
  fgetc(in);
  // The next four bytes are the number of colors in the palette of the image
  // and are ignored in this context.
  fgetc(in);
  fgetc(in);
  fgetc(in);
  fgetc(in);
  // The next four bytes are the number of "important" colors of the image and
  // are ignored in this context.
  fgetc(in);
  fgetc(in);
  fgetc(in);
  fgetc(in);
  // If the DIB header is somehow longer than the 40 bytes that have already
  // been read, read and discard the rest of them.
  ...
  printf("%d data bytes\n", bmpDataSize);
  // The remaining contents are color data and need to be converted from 24-bit
