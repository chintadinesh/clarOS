// -------------------------------------------------------------------
// File name: Blynk.c
// Description: This code is used to bridge the TM4C123 board and the Blynk
// Application
...
// Use LM2937-3.3 and two 4.7 uF capacitors to convert USB +5V to 3.3V for the
// ESP8266 http://www.ti.com/lit/ds/symlink/lm2937-3.3.pdf
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
// ----------------------------------- TM4C_to_Blynk
// ------------------------------ Send data to the Blynk App It uses Virtual Pin
// numbers between 70 and 99 so that the ESP8266 knows to forward the data to
// the Blynk App
...
// -------------------------   Blynk_to_TM4C -----------------------------------
// This routine receives the Blynk Virtual Pin data via the ESP8266 and parses
// the data and feeds the commands to the TM4C.
void Blynk_to_TM4C(void) {
  ...
  if (ESP8266_GetMessage(serial_buf)) {  // returns false if no message
    ...
    // Rip the 3 fields out of the CSV data. The sequence of data from the 8266
    // is: Pin #, Integer Value, Float Value.
    strcpy(Pin_Number, strtok(serial_buf, ","));
    strcpy(Pin_Integer,
           strtok(NULL,
                  ","));  // Integer value that is determined by the Blynk App
