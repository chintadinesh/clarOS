...
// create your own Blynk server app and edit this next line with your
// authentication code
...
char Rx5Fifo[RX5FIFOSIZE][MESSAGESIZE];  // each message can have up to
                                         // MESSAGESIZE-1 characters
...
// 0 to RX5FIFOSIZE-1
uint32_t Rx5Fifo_Size(void) { return ((uint32_t)(Rx5PutI - Rx5GetI)); }
...
uint32_t ESP8266_AvailableInput(void) { return Rx5Fifo_Size(); }
...
#define PE3             \
  (*((volatile uint32_t \
          *)0x40024020))  // LED to indicate that the 8266 connected
...
// hotspot. Enable the DEBUG flags in esp8266.h if you want to watch the
// transactions.
