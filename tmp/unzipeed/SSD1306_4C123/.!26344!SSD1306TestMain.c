...
int main0(void) {
  ...
  for (count = 0; count < 21; count = count + 1) {
    ...
    for (i = 0; i < 3; i = i + 1) {
      for (j = 0; j < 5; j = j + 1) {
        ...
        SSD1306_DrawBMP(5 + 21 * j + count, 10 + 11 * i, SmallEnemy, 0,
                        SSD1306_WHITE);
      }
    }
    SSD1306_OutBuffer();
  ...
  while (1) {
    ...
    SSD1306_OutUHex7(letter);
    SSD1306_OutString(" = ");
    SSD1306_OutChar(letter);
    Clock_Delay1ms(500);
    data = data + 27;
    sdat = -1 * (sdat + 27);
    letter++;
...
void SSD1306_LanguageTest(Language_t l) {
  ...
  myLanguage = l;
  SSD1306_OutPhrase(LANGUAGE);
  SSD1306_OutChar(' ');
  SSD1306_OutPhrase(HELLO);
  SSD1306_OutChar(' ');
  SSD1306_OutPhrase(GOODBYE);
  SSD1306_OutChar(13);
  myLanguage = old;
}
...
void Draw1(void) {
  ...
  if (NeedToDraw) {
    ...
    for (i = 0; i < 4; i++) {
      if (Enemys1[i].life) {
        SSD1306_DrawBMP(Enemys1[i].x, Enemys1[i].y, Enemys1[i].image, 0,
                        SSD1306_INVERSE);
      }
    }
    SSD1306_OutBuffer();
...
void Move2(void) {
  ...
  for (i = 0; i < 5; i++) {
    if (Enemys[i].life == alive) {
      ...
      if ((Enemys[i].y > 62) || (Enemys[i].y < 10) || (Enemys[i].x < 0) ||
          (Enemys[i].x > 118)) {
...
void Draw(void) {
  ...
  for (i = 0; i < MAX; i++) {
    if (Enemys[i].life == alive) {
      SSD1306_DrawBMP(Enemys[i].x, Enemys[i].y, Enemys[i].image, 0,
                      SSD1306_INVERSE);
    }
  }
  SSD1306_OutBuffer();
