...
int main(void) {
  ...
  //  PWM_Init(320, 256);             // initialize timer0A in PWM mode (50,000
  //  Hz, 80% duty cycle) PWM_Init(800, 760) ;            // initialize timer0A
  //  in PWM mode (20,000 Hz, 95% duty cycle) PWM_Init(1067, 160);            //
  //  initialize timer0A in PWM mode (15,000 Hz, 15% duty cycle) PWM_Init(16000,
  //  12000);         // initialize timer0A in PWM mode (1,000 Hz, 75% duty
  //  cycle) PWM_Init(16000, 8000);          // initialize timer0A in PWM mode
  //  (1,000 Hz, 50% duty cycle)
  PWM_Init(16000,
           4000);  // initialize timer0A in PWM mode (1,000 Hz, 25% duty cycle)
  //  PWM_Init(2, 1);          // initialize timer0A in PWM mode (8MHz, 50% duty
  //  cycle)
  while (1) {
    WaitForInterrupt();
  }
